...
void initialize_dynamic_allocator(uint32 daStart,
  ...
  // TODO: [PROJECT'24.MS1 - #04] [3] DYNAMIC ALLOCATOR -
  // initialize_dynamic_allocator COMMENT THE FOLLOWING LINE BEFORE START CODING
...
void set_block_data(void *va, uint32 totalSize, bool isAllocated) {
  ...
  *head_ptr = Calc_size;
  uint32 *footer_ptr =
      va +
      (totalSize - 8); // footer_ptr points to footer start (remove the header)
...
void *alloc_block_FF(uint32 size) {
  ...
  //==================================================================================
  {
    ...
    if (!is_initialized) {
      ...
      uint32 da_start =
          (uint32)sbrk(ROUNDUP(required_size, PAGE_SIZE) / PAGE_SIZE);
  ...
  LIST_FOREACH(iter_element, &freeBlocksList) {
    ...
    if (blk_size >= 16 && blk_size >= Totalsize) // law makan yekafy
    {
      ...
      } else if (blk_size - Totalsize < 16) // internal frag
      {
        ...
        // \n",(void*)iter_element); cprintf("the address of the iter_elemnt
        // 0x%x \n",iter_element);
      ...
      {
        struct BlockElement *new_element =
            (struct BlockElement *)((char *)iter_element + Totalsize);
  ...
  if (found == 0) {
    ...
    // \n",get_block_size(sbrk_blk),is_free_block(sbrk_blk)); panic("ooo");
    if (sbrk_blk == (void *)-1) {
      // cprintf("awl if \n");
      return NULL;
    } else {
      uint32 *New_End =
          (uint32 *)((char *)sbrk_blk +
                     (ROUNDUP(Totalsize, PAGE_SIZE) / PAGE_SIZE) * PAGE_SIZE -
                     sizeof(int));
      *New_End = 1;
      set_block_data(
          sbrk_blk, (ROUNDUP(Totalsize, PAGE_SIZE) / PAGE_SIZE) * PAGE_SIZE, 0);
      ...
      if (freee == 0) { // allocated
        if (LIST_SIZE(&freeBlocksList) == 0) {
          LIST_INSERT_HEAD(&freeBlocksList,
                           (struct BlockElement *)((char *)sbrk_blk));
        } else {
          LIST_INSERT_TAIL(&freeBlocksList,
                           (struct BlockElement *)((char *)sbrk_blk));
        }
      } else { // free
        uint32 *pointer_to_free_header =
            (uint32 *)((char *)footer_of_block -
                       get_block_size(footer_of_block) + sizeof(int));
        set_block_data(pointer_to_free_header,
                       get_block_size(pointer_to_free_header) +
                           get_block_size(sbrk_blk),
                       0);
        struct BlockElement *mergedblock =
            (struct BlockElement *)pointer_to_free_header;
        LIST_INSERT_AFTER(&freeBlocksList,
                          (struct BlockElement *)pointer_to_free_header,
                          mergedblock);
...
void *alloc_block_BF(uint32 size) {
  ...
  uint32 bestFitBlockSize =
      4294967295U; // variable keber fel awel aashan nqaren bih
  uint32 blk_size;
  ...
  if (bestFitBlock == NULL)
    ...
    } else if (bestFitBlockSize - Totalsize < 16) // internal frag
    {
      ...
      // cprintf("the address of the iter_elemnt void 0x%x
      // \n",(void*)iter_element); cprintf("the address of the iter_elemnt 0x%x
      // \n",iter_element);
    ...
    {
      struct BlockElement *new_element =
          (struct BlockElement *)((char *)bestFitBlock + Totalsize);
      // pushing it it the free block list
...
void free_block(void *va) {
  ...
  // handling if the va isn't allocated "masking to get least significant bit to
  // know if it is allocated or not"
  ...
  // we will see if the before and after blocks is allocated or not by footer or
  // header"check if the size 0 and alloc 1"heap boundaries"
  ...
  uint32 *prev_blockptr_header =
      (uint32 *)((char *)prev_blockptr_footer - (char *)size_of_prev_block + 4);
  //////////////////////////////////////////////////////////////
  ...
  if (prev_blockptr_header != NULL && next_blockptr_header != NULL) {
    ...
    if ((notallocatedprev == 0) && (notallocatednext == 0)) {
      ...
      struct BlockElement *nextblock =
          (struct BlockElement *)next_blockptr_header;
      ...
      struct BlockElement *prevblock = (struct BlockElement *)
          prev_blockptr_header; // to be removed from the freeblocklist
      LIST_REMOVE(&freeBlocksList, prevblock);
      struct BlockElement *block;
      struct BlockElement *mergedblock = (struct BlockElement *)
          prev_blockptr_header; // to be added in the freeblocklist
      bool flag = 0;
      if (LIST_SIZE(&freeBlocksList) == 0) {
      ...
      uint32 newsize =
          size_of_current_block + size_of_next_block + size_of_prev_block;
      set_block_data(prev_blockptr_header, newsize, 0);
      struct BlockElement *mergedblock =
          (struct BlockElement *)prev_blockptr_header;
      struct BlockElement *prevblock =
          (struct BlockElement *)prev_blockptr_header;
      struct BlockElement *nextblock =
          (struct BlockElement *)next_blockptr_header;
      LIST_REMOVE(&freeBlocksList, prevblock);
  ...
  if (TotalNewSize <= current_block_size) {
    ...
    if (freeSpace >= 16) {
      ...
      if (is_free_block(next_blockptr_header)) {
        char *address_new_free_element = va + TotalNewSize;
        set_block_data(va, TotalNewSize, 1);
        set_block_data(address_new_free_element,
                       size_of_next_block + (current_block_size)-TotalNewSize,
                       0);
        LIST_INSERT_BEFORE(&freeBlocksList,
                           (struct BlockElement *)next_blockptr_header,
                           (struct BlockElement *)address_new_free_element);
        LIST_REMOVE(&freeBlocksList,
                    (struct BlockElement *)next_blockptr_header);
      } else
