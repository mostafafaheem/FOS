...
struct semaphore create_semaphore(char *semaphoreName, uint32 value) {
  ...
  // struct semaphore* temp_sem=(struct semaphore*)malloc(sizeof(struct
  // semaphore));
  struct semaphore temp_sem;
  temp_sem.semdata =
      (struct __semdata *)smalloc(semaphoreName, sizeof(struct __semdata), 1);
...
int semaphore_count(struct semaphore sem) { return sem.semdata->count; }
