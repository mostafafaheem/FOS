...
//	if (((r = inb(0x1F7)) & (IDE_BSY|IDE_DRDY)) != IDE_DRDY)
//	{
//		//should sleep (i.e. blocked) until a IRQ14 (Primary IDE)
//interrupt occur 		if (DISK_INT_BLK_METHOD == LCK_SLEEP)
//		{
//			acquire_spinlock(&DISKlock);
//			{
//				cprintf("\n[%d] Will be BLOCKED\n",
//get_cpu_proc()->env_id); 				sleep(&DISKchannel, &DISKlock);
//			}
...
//		LOG_STATMENT(cprintf("ERROR @ ide_wait_ready() =
//%x(%d)\n",r,r);); 		return -1;
//	}
//	return 0;
// }
...
int ide_read(uint32 secno, void *dst, uint32 nsecs) {
  ...
  // FUTURE NOTE: This BUSY-WAIT should be replaced by Interrupt to allow the OS
  // to schedule another process till the device become ready [el7 :)]
  ide_wait_ready(0);
...
int ide_write(uint32 secno, const void *src, uint32 nsecs) {
  ...
  for (; nsecs > 0; nsecs--, src += SECTSIZE) {
    if ((r = ide_wait_ready(1)) < 0) {
      ...
      // LOG_STATMENT(cprintf("written %d sectors to disk
      // successfully\n",nsecs););
    }
  }
