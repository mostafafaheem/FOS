...
struct Page_Info arrPageInfo[(USER_HEAP_MAX - USER_HEAP_START) / PAGE_SIZE] = {
    0};
// struct id_search id_search_array[(USER_HEAP_MAX - USER_HEAP_START) /
// PAGE_SIZE]={ 0 };
...
//=============================================
/*2023*/
void *sbrk(int increment) { return (void *)sys_sbrk(increment); }
    ...
    if (size <= DYN_ALLOC_MAX_BLOCK_SIZE) {
      ...
      while (increment < no_of_pages) {
        ...
        if (arrPageInfo[index].marked == 0) {
          ...
          increment = 0;
          startIndex = index + ((ROUNDUP(arrPageInfo[index].size, PAGE_SIZE)) /
                                PAGE_SIZE);
          index = startIndex;
      ...
      for (int i = 0; i < no_of_pages; i++) {
        ...
        arrPageInfo[markingCounter].virtual_address =
            (markingCounter * PAGE_SIZE) + USER_HEAP_START;
...
void free(void *virtual_address) {
  ...
  if ((uint32)virtual_address < USER_HEAP_START ||
    ...
    uint32 size_allocated =
        arrPageInfo[((uint32)virtual_address - USER_HEAP_START) / PAGE_SIZE]
            .size;
    // cprintf("1\n");
    ...
    uint32 start_index =
        ((uint32)virtual_address - USER_HEAP_START) / PAGE_SIZE;
    // cprintf("4\n");
  ...
  if (sys_isUHeapPlacementStrategyFIRSTFIT()) {
    ...
    while (increment < no_of_pages) {
      if (arrPageInfo[index].marked == 0) {
        ...
        increment = 0;
        startIndex =
            index + ((ROUNDUP(arrPageInfo[index].size, PAGE_SIZE)) / PAGE_SIZE);
        index = startIndex;
      }
      if ((index * PAGE_SIZE) + USER_HEAP_START >= USER_HEAP_MAX) {
        return NULL;
    ...
    if (ret == E_SHARED_MEM_EXISTS) {
      ...
      idVAR.id = ret;
      // cprintf("id = %d \n",idVAR.id);
      //		for(int i=0;i<(USER_HEAP_MAX - USER_HEAP_START) /
      //PAGE_SIZE;i++)
      //			{
      //
      //				if(id_search_array[i].id==0 &&
      //id_search_array[i].va==0)
      //				{
...
//	To do this, we need to switch to the kernel, free the pages AND "EMPTY"
//PAGE TABLES 	from main memory then switch back to the user again.
//
//	use sys_freeSharedObject(...); which switches to the kernel mode,
//	calls freeSharedObject(...) in "shared_memory_manager.c", then switch
//back to the user mode here 	the freeSharedObject() function is empty, make sure
//to implement it.
...
void sfree(void *virtual_address) {
  ...
  //		}
  sys_freeSharedObject(
      arrPageInfo[((uint32)virtual_address - USER_HEAP_START) / PAGE_SIZE].id,
      virtual_address);
}
...
//new virtual_address, in which case the old virtual_address must no longer be
//accessed. 	On failure, returns a null pointer, and the old virtual_address
//remains valid.
...
//		in "kern/mem/chunk_operations.c", then switch back to the user
//mode here 	the move_user_mem() function is empty, make sure to implement it.
...
void expand(uint32 newSize) { panic("Not Implemented"); }
void shrink(uint32 newSize) { panic("Not Implemented"); }
void freeHeap(void *virtual_address) { panic("Not Implemented"); }
