...
void FOS_initialize() {
  ...
  // cprintf("*	1) Global data (BSS) section...");
  {
    ...
    // clear the uninitialized global data (BSS) section of our program, from
    // start_of_uninitialized_data_section to end_of_kernel This ensures that
    // all static/global variables start with zero value.
    memset(start_of_uninitialized_data_section, 0,
           end_of_kernel - start_of_uninitialized_data_section);
  }
  // cprintf("[DONE]\n");
  ...
  {
    // Relocate SP to its corresponding location in the specific stack area
    // below KERN_BASE (SCHD_KERN_STACK_TOP)
...
dead:
  ...
  if (cur_env != NULL && cur_env->env_status == ENV_RUNNING) {
    ...
    env_exit(); // env_exit --> sched_exit_env --> sched --> context_switch into
                // fos_scheduler
  }
...
/*
 * Panic is called on unresolvable fatal errors.
 * It prints "panic: mesg", exit all env's and then enters the kernel command
 * prompt.
 */
...
dead:
  ...
  if (sched_stat != SCH_UNINITIALIZED) {
    ...
    if (cur_env != NULL && cur_env->env_status == ENV_RUNNING) {
      // cprintf("exit curenv...........\n");
      // Place the running env into the exit queue then switch to the scheduler
      env_exit(); // env_exit --> sched_exit_env --> sched --> context_switch
                  // into fos_scheduler
    }
  }
  // else //2024: panic from Kernel and no current running env
...
/*
 * Panic is called on unresolvable fatal errors.
 * It prints "panic: mesg", exit the curenv (if any) and break into the command
 * prompt.
 */
