...
void test_bsd_nice_0() {
  if (firstTimeTestBSD) {
    ...
    cprintf("> Running... (After all running programs finish, Run the same "
            "command again.)\n");
...
void test_bsd_nice_1() {
  if (firstTimeTestBSD) {
    ...
    if (fibEnv == NULL || fibposnEnv == NULL || fibnegnEnv == NULL)
      panic("Loading programs failed\n");
    if (fibEnv->page_WS_max_size != 500 ||
        fibposnEnv->page_WS_max_size != 500 ||
        fibnegnEnv->page_WS_max_size != 500)
      panic("The programs should be initially loaded with the given working "
            "set size. fib: %d, fibposn: %d, fibnegn: %d\n",
            fibEnv->page_WS_max_size, fibposnEnv->page_WS_max_size,
            fibnegnEnv->page_WS_max_size);
    ...
    acquire_spinlock(&ProcessQueues.qlock);
    {
      ...
      // LIST_FOREACH_R(env, &env_exit_queue)
      {
        ...
        // env->env_id, env->priority, env->nice);
