...
uint32 allocSizes[numOfAllocs] = {4 * kilo, 20 * sizeof(char) + sizeOfMetaData,
                                  1 * kilo, 3 * sizeof(int) + sizeOfMetaData,
                                  2 * kilo, 2 * sizeOfMetaData,
                                  7 * kilo};
short *startVAs[numOfAllocs * allocCntPerSize + 1];
short *midVAs[numOfAllocs * allocCntPerSize + 1];
short *endVAs[numOfAllocs * allocCntPerSize + 1];

int check_block(void *va, void *expectedVA, uint32 expectedSize,
                uint8 expectedFlag) {
  // Check returned va
  if (va != expectedVA) {
...
void test_initialize_dynamic_allocator() {
#if USE_KHEAP
  panic("test_initialize_dynamic_allocator: the kernel heap should be diabled. "
        "make sure USE_KHEAP = 0");
  return;
#endif
  ...
  *(tmp_ptr + 1) = 1;
  ...
  uint32 *daEnd =
      (uint32 *)(KERNEL_HEAP_START + initAllocatedSpace - sizeof(int));
  uint32 *blkHeader = (uint32 *)(KERNEL_HEAP_START + sizeof(int));
  uint32 *blkFooter =
      (uint32 *)(KERNEL_HEAP_START + initAllocatedSpace - 2 * sizeof(int));
  if (*daBeg != 1 || *daEnd != 1 ||
      (*blkHeader != initAllocatedSpace - 2 * sizeof(int)) ||
      (*blkFooter != initAllocatedSpace - 2 * sizeof(int))) {
    panic("Content of header/footer and/or DA begin/end are not set correctly");
  }
  if (LIST_SIZE(&freeBlocksList) != 1 ||
      (uint32)LIST_FIRST(&freeBlocksList) !=
          KERNEL_HEAP_START + 2 * sizeof(int)) {
    panic("free block is not added correctly");
  }

  cprintf("Congratulations!! test initialize_dynamic_allocator completed "
          "successfully.\n");
}

int test_initial_alloc(int ALLOC_STRATEGY) {
#if USE_KHEAP
  panic("test_initial_alloc: the kernel heap should be disabled. make sure "
        "USE_KHEAP = 0");
  return 0;
#endif
  ...
  void *va;
  //====================================================================//
  /*INITIAL ALLOC Scenario 1: Try to allocate a block with a size greater than
   * the size of any existing free block*/
  ...
  //====================================================================//
  /*INITIAL ALLOC Scenario 2: Try to allocate set of blocks with different
   * sizes*/
  cprintf("	2: Try to allocate set of blocks with different sizes [all "
          "should fit]\n\n");
  is_correct = 1;
  void *expectedVA;
  ...
  int remainSize = initAllocatedSpace - totalSizes -
                   2 * sizeof(int); // exclude size of "DA Begin & End" blocks
  // cprintf("\n********* Remaining size = %d\n", remainSize);
  if (remainSize <= 0) {
    is_correct = 0;
    cprintf(
        "alloc_block_xx test is not configured correctly. Consider updating "
        "the initial allocated space OR the required allocations\n");
  }
  int idx = 0;
  void *curVA = (void *)KERNEL_HEAP_START +
                sizeof(int); // just after the "DA Begin" block
  uint32 actualSize;
  for (int i = 0; i < numOfAllocs; ++i) {
  ...
  //====================================================================//
  /*INITIAL ALLOC Scenario 3: Try to allocate a block with a size equal to the
   * size of the first existing free block*/
  cprintf("	3: Try to allocate a block with equal to the first existing "
          "free block\n\n");
  is_correct = 1;
  ...
  *(endVAs[idx]) = idx;
  ...
  for (int i = 0; i < idx; ++i) {
    if (*(startVAs[i]) != i || *(midVAs[i]) != i || *(endVAs[i]) != i) {
      is_correct = 0;
      cprintf("alloc_block_xx #4.%d: WRONG! content of the block is not "
              "correct. Expected %d\n",
              i, i);
      break;
    }
...
void test_alloc_block_FF() {
#if USE_KHEAP
  panic("test_alloc_block_FF: the kernel heap should be disabled. make sure "
        "USE_KHEAP = 0");
  return;
#endif
  ...
  //====================================================================//
  /*FF ALLOC Scenario 1: Try to allocate a block with a size greater than the
   * size of any existing free block*/
  cprintf("	1: Try to allocate large block [not fit in any space]\n\n");
  is_correct = 1;
  ...
  if (va != NULL) {
    is_correct = 0;
    cprintf("alloc_block_FF #5: WRONG FF ALLOC - alloc_block_FF find a block "
            "instead no existing free blocks with the given size.\n");
  }
  ...
  //====================================================================//
  /*FF ALLOC Scenario 2: Try to allocate blocks with sizes smaller than existing
   * free blocks*/
  cprintf("	2: Try to allocate set of blocks with different sizes smaller "
          "than existing free blocks\n\n");
  is_correct = 1;
  void *expectedVA;
  uint32 expectedNumFreeBlks;
#define numOfFFTests 3
  uint32 startVA =
      KERNEL_HEAP_START + sizeof(int); // just after the DA Begin block
  ...
  uint32 expectedVAs[numOfFFTests] = {
      startVA + sizeOfMetaData / 2, startOf1st7KB,
      startVA + testSizes[0] + sizeOfMetaData / 2};
  ...
  //====================================================================//
  /*FF ALLOC Scenario 3: Try to allocate a block with a size equal to the size
   * of the first existing free block*/
  cprintf("	3: Try to allocate a block with equal to the first existing "
          "free block\n\n");
  is_correct = 1;
  ...
  // Check block
  expectedVA =
      (void *)(startVA + testSizes[0] + testSizes[2] + sizeOfMetaData / 2);
  ...
  *(tstEndVAs[numOfFFTests]) = 353;
  ...
  //====================================================================//
  /*FF ALLOC Scenario 4: Try to allocate a block with a bit smaller size
   * [internal fragmentation case]*/
  cprintf("	4: Try to allocate a block with a bit smaller size [internal "
          "fragmentation case]\n\n");
  is_correct = 1;
  ...
  *(tstEndVAs[numOfFFTests + 1]) = 353;
  ...
  for (int i = 0; i < numOfFFTests + 2; ++i) {
    ...
    // tstMidVAs[i], tstEndVAs[i]);
    if (*(tstStartVAs[i]) != 353 || *(tstMidVAs[i]) != 353 ||
        *(tstEndVAs[i]) != 353) {
      is_correct = 0;
      cprintf("alloc_block_FF #8.%d: WRONG! content of the block is not "
              "correct. Expected=%d, val1=%d, val2=%d, val3=%d\n",
...
void test_alloc_block_BF() {
#if USE_KHEAP
  panic("test_alloc_block_BF: the kernel heap should be disabled. make sure "
        "USE_KHEAP = 0");
  return;
#endif
  ...
  //====================================================================//
  /*BF ALLOC Scenario 1: Try to allocate a block with a size greater than the
   * size of any existing free block*/
  cprintf("	1: Try to allocate large block [not fit in any space]\n\n");
  is_correct = 1;
  ...
  if (va != NULL) {
    is_correct = 0;
    cprintf("alloc_block_BF #5: WRONG BF ALLOC - alloc_block_BF find a block "
            "instead no existing free blocks with the given size.\n");
  }
  if (is_correct) {
    eval += 5;
  ...
  //====================================================================//
  /*BF ALLOC Scenario 2: Try to allocate blocks with sizes smaller than existing
   * free blocks*/
  cprintf("	2: Try to allocate set of blocks with different sizes smaller "
          "than existing free blocks\n\n");
  is_correct = 1;
...
#define numOfBFTests 5
  uint32 testSizes[numOfBFTests] = {/*only 1 can fit*/ 4 * kilo + kilo / 2,
  ...
  uint32 expectedVAs[numOfBFTests] = {
      startOf1st7KB, startOf1st2KB, startOf1st2KB + testSizes[1], startOf1st1KB,
      KERNEL_HEAP_START + 2 * sizeof(int)};
  ...
  //====================================================================//
  /*BF ALLOC Scenario 3: Try to allocate a block with a size equal to the size
  ...
  *(tstEndVAs[numOfBFTests + 1]) = 353;
  ...
  for (int i = 0; i < numOfBFTests + 2; ++i) {
    ...
    if (*(tstStartVAs[i]) != 353 || *(tstMidVAs[i]) != 353 ||
      ...
      // tstMidVAs[i], tstEndVAs[i]);
      is_correct = 0;
      cprintf("alloc_block_BF #8.%d: WRONG! content of the block is not "
              "correct. Expected=%d, val1=%d, val2=%d, val3=%d\n",
              i, 353, *(tstStartVAs[i]), *(tstMidVAs[i]), *(tstEndVAs[i]));
      break;
    }
...
void test_alloc_block_NF() {
  ...
  //====================================================================//
  /*NF ALLOC Scenario 1: Try to allocate a block with a size greater than the
   * size of any existing free block*/

  //====================================================================//
  /*NF ALLOC Scenario 2: Try to allocate a block with a size equal to the size
   * of the one existing free blocks (STARTING from 0)*/

  //====================================================================//
  /*NF ALLOC Scenario 3: Try to allocate a block with a size equal to the size
   * of the one existing free blocks (The first one fit after the last allocated
   * VA)*/

  //====================================================================//
  /*NF ALLOC Scenario 4: Try to allocate a block with a size smaller than the
   * size of any existing free block (The first one fit after the last allocated
   * VA)*/

  //====================================================================//
  /*NF ALLOC Scenario 5: Try to allocate a block with a size smaller than the
   * size of any existing free block (One from the updated blocks before in the
   * free list)*/

  //====================================================================//
  /*NF ALLOC Scenario 6: Try to allocate a block with a size smaller than ALL
   * the NEXT existing blocks .. Shall start search from the start of the list*/

  //====================================================================//
  /*NF ALLOC Scenario 7: Try to allocate a block with a size smaller than the
   * existing blocks .. To try to update head not to remove it*/
...
void test_free_block_FF() {
...
#if USE_KHEAP
  panic("test_free_block: the kernel heap should be disabled. make sure "
        "USE_KHEAP = 0");
  return;
#endif
  ...
  cprintf("	1: Try to allocate set of blocks with different sizes to "
          "fill-up the allocated space\n\n");
  ...
  int remainSize = initAllocatedSpace - totalSizes -
                   2 * sizeof(int); // exclude size of "DA Begin & End" blocks
  if (remainSize <= 0)
    panic("test_free_block is not configured correctly. Consider updating the "
          "initial allocated space OR the required allocations");
  ...
  for (int i = 0; i < numOfAllocs; ++i) {
    for (int j = 0; j < allocCntPerSize; ++j) {
      ...
      if (check_block(va, expected_va, allocSizes[i], 1) == 0)
        ...
        panic("test_free_block #1.%d: WRONG ALLOC - alloc_block_FF return "
              "wrong address. Expected %x, Actual %x",
              idx, expected_va, va);
      curVA += allocSizes[i];
      *(startVAs[idx]) = idx;
      *(midVAs[idx]) = idx;
  ...
  //====================================================================//
  /* Try to allocate a block with a size equal to the size of the first existing
   * free block*/
  actualSize = remainSize - sizeOfMetaData;
  va = startVAs[idx] = alloc_block(actualSize, DA_FF);
  midVAs[idx] = va + actualSize / 2;
  ...
  if (check_block(va, expected_va, remainSize, 1) == 0)
    //			if(va != (curVA + sizeOfMetaData/2))
    panic("test_free_block #2: WRONG ALLOC - alloc_block_FF return wrong "
          "address. Expected %x, Actual %x",
          expected_va, va);
  *(startVAs[idx]) = idx;
  *(midVAs[idx]) = idx;
  *(endVAs[idx]) = idx;
  ...
  for (int i = 0; i < idx; ++i) {
    if (*(startVAs[i]) != i || *(midVAs[i]) != i || *(endVAs[i]) != i) {
      is_correct = 0;
      cprintf("test_free_block #3.%d: WRONG! content of the block is not "
              "correct. Expected %d\n",
              i, i);
      break;
    }
  ...
  for (int i = 0; i < numOfAllocs; ++i) {
    cprintf("test#%d\n", i);
    free_block(startVAs[i * allocCntPerSize]);
    if (check_block(startVAs[i * allocCntPerSize],
                    startVAs[i * allocCntPerSize], allocSizes[i], 0) == 0) {
      is_correct = 0;
    }
  }
  ...
  if (is_correct)
    is_correct =
        check_block(startVAs[numOfAllocs * allocCntPerSize],
                    startVAs[numOfAllocs * allocCntPerSize], remainSize, 0);
  ...
  if (is_correct)
    is_correct = check_block(startVAs[numOfAllocs * allocCntPerSize - 1],
                             startVAs[numOfAllocs * allocCntPerSize - 1],
                             allocSizes[numOfAllocs - 1], 0);
  ...
  if (check_block(startVAs[blockIndex - 1], startVAs[blockIndex - 1],
                  expected_size, 0) == 0) {
    is_correct = 0;
  }
  //====================================================================//
  ...
  if (check_block(startVAs[blockIndex], startVAs[blockIndex], expected_size,
                  0) == 0) {
    is_correct = 0;
  }
  ...
  cprintf(
      "	6: Free some allocated blocks [Merge with previous & next]\n\n");
  is_correct = 1;
  ...
  // Check returned va
  void *expected =
      (void *)(KERNEL_HEAP_START + sizeof(int) + sizeOfMetaData / 2);
  ...
  // Check returned va
  expected =
      (void *)(KERNEL_HEAP_START + sizeof(int) + 5 * kilo + sizeOfMetaData / 2);
  if (check_block(va, expected, expected_size, 1) == 0) {
    is_correct = 0;
  ...
  expected_size = MAX(ROUNDUP(actualSize + sizeOfMetaData, 2),
                      allocSizes[0] + allocSizes[1]);
  va = alloc_block(actualSize, DA_FF);
  // Check returned va
...
void test_free_block_BF() {
  ...
  for (int i = 0; i < numOfAllocs; ++i) {
    for (int j = 0; j < allocCntPerSize; ++j) {
      ...
      expected_va = curVA + sizeOfMetaData / 2;
      if (check_block(va, expected_va, allocSizes[i], 1) == 0)
        panic("test_free_block #1.%d: WRONG ALLOC - alloc_block_BF return "
              "wrong address. Expected %x, Actual %x",
  ...
  expected_va = curVA + sizeOfMetaData / 2;
  if (check_block(va, expected_va, remainSize, 1) == 0)
    panic("test_free_block #1: WRONG ALLOC - alloc_block_BF return wrong "
          "address.");
  *(startVAs[idx]) = idx;
  *(midVAs[idx]) = idx;
  *(endVAs[idx]) = idx;
  ...
  for (int i = 0; i < idx; ++i) {
    if (*(startVAs[i]) != i || *(midVAs[i]) != i || *(endVAs[i]) != i)
      panic("test_free_block #2.%d: WRONG! content of the block is not "
            "correct. Expected %d",
            i, i);
  }
  ...
  // Free last block
  free_block(startVAs[numOfAllocs * allocCntPerSize]);
  if (check_block(startVAs[numOfAllocs * allocCntPerSize],
                  startVAs[numOfAllocs * allocCntPerSize], remainSize, 0) == 0)
    panic("3.3 Failed");
  ...
  // Free block before last
  free_block(startVAs[numOfAllocs * allocCntPerSize - 1]);
  if (check_block(startVAs[numOfAllocs * allocCntPerSize - 1],
                  startVAs[numOfAllocs * allocCntPerSize - 1],
                  allocSizes[numOfAllocs - 1], 0) == 0)
    panic("3.5 Failed");
  ...
  cprintf("		7.3: in block coalesces with NEXT [INTERNAL "
          "FRAGMENTATION]\n\n");
  actualSize = 4 * kilo + 10;
  expected_size =
      allocSizes[0] + allocSizes[1]; // ROUNDUP(actualSize + sizeOfMetaData,2) ;
  va = alloc_block(actualSize, DA_BF);
  // Check returned va
  expected = startVAs[1 * allocCntPerSize - 1];
  if (check_block(va, expected, expected_size, 1) == 0) {
  ...
  cprintf("Congratulations!! test free_block with BEST FIT completed "
          "successfully.\n");
}

void test_free_block_NF() { panic("not implemented"); }
...
void test_realloc_block_FF() {
  ...
  // TODO: [PROJECT'24.MS1 - #09] [3] DYNAMIC ALLOCATOR -
  // test_realloc_block_FF() CHECK MISSING CASES AND TRY TO TEST THEM !
  ...
  int remainSize = initAllocatedSpace - totalSizes -
                   2 * sizeof(int); // exclude size of "DA Begin & End" blocks
  if (remainSize <= 0)
    panic("test_realloc_block_FF is not configured correctly. Consider "
          "updating the initial allocated space OR the required allocations");
  ...
  for (int i = 0; i < numOfAllocs; ++i) {
    for (int j = 0; j < allocCntPerSize; ++j) {
      ...
      if (check_block(va, expectedVA, expectedSize, 1) == 0) {
        panic("test_realloc_block_FF #1.1.%d: WRONG ALLOC - it return wrong "
              "address. Expected %x, Actual %x",
              idx, curVA + sizeOfMetaData, va);
      }
      curVA += allocSizes[i];
  ...
  if (check_block(va, expectedVA, expectedSize, 1) == 0) {
    panic(
        "test_realloc_block_FF #1.2.0: WRONG ALLOC - it return wrong address.");
  }
  *(startVAs[idx]) = idx;
  *(midVAs[idx]) = idx;
  *(endVAs[idx]) = idx;
  ...
  for (int i = 0; i < idx; ++i) {
    if (*(startVAs[i]) != i || *(midVAs[i]) != i || *(endVAs[i]) != i)
      panic("test_realloc_block_FF #1.3.%d: WRONG! content of the block is not "
            "correct. Expected %d",
            i, i);
  }
  ...
  for (int i = 0; i < numOfAllocs; ++i) {
    ...
    expectedVA = va;
    if (check_block(startVAs[i * allocCntPerSize],
                    startVAs[i * allocCntPerSize], expectedSize, 0) == 0) {
  ...
  for (int i = 0; i < idx; ++i) {
    if (i % allocCntPerSize == 0)
      continue;
    if (*(startVAs[i]) != i || *(midVAs[i]) != i || *(endVAs[i]) != i)
      panic("test_realloc_block_FF #2.4.%d: WRONG! content of the block is not "
            "correct. Expected %d",
            i, i);
  }
  ...
  {
    blockIndex = 4 * allocCntPerSize - 1;
    new_size = allocSizes[3] /*12+16 B*/ + allocSizes[4] / 2 /*2KB/2*/ -
               sizeOfMetaData;
    ...
    if (*(startVAs[blockIndex]) != blockIndex ||
      ...
      is_correct = 0;
      cprintf("test_realloc_block_FF #3.1.2: WRONG REALLOC! content of the "
              "block is not correct. Expected %d\n",
              blockIndex);
    }
  }
  if (is_correct) {
  ...
  {
    blockIndex = 4 * allocCntPerSize - 1;
    // new_size = allocSizes[3] /*12+16B + 2KB/2*/ + allocSizes[4]/2 /*2KB/2*/ -
    // sizeOfMetaData;
    ...
    if (*(startVAs[blockIndex]) != blockIndex ||
        ...
        *(endVAs[blockIndex]) != blockIndex) {
      is_correct = 0;
      cprintf("test_realloc_block_FF #3.2.2: WRONG REALLOC! content of the "
              "block is not correct. Expected %d\n",
              blockIndex);
    }
  ...
  is_correct = 1;
  {
    ...
    struct BlockElement *newBlkAddr =
        (struct BlockElement *)(va + new_size + 2 * sizeof(int));
    ...
    if (*(startVAs[blockIndex]) != blockIndex ||
        *(midVAs[blockIndex]) != blockIndex) {
      is_correct = 0;
      cprintf("test_realloc_block_FF #4.1.3: WRONG REALLOC! content of the "
              "block is not correct. Expected %d\n",
              blockIndex);
    }
  ...
  cprintf(
      "	4.2: next block is full (NO coalesce) [Internal Fragmentation]\n\n");
  ...
  is_correct = 1;
  {
    ...
    if (*(startVAs[blockIndex]) != blockIndex ||
        *(midVAs[blockIndex]) != blockIndex) {
      is_correct = 0;
      cprintf("test_realloc_block_FF #4.2.2: WRONG REALLOC! content of the "
              "block is not correct. Expected %d\n",
              blockIndex);
    }
  ...
  cprintf("[PARTIAL] test realloc_block with FIRST FIT completed. Evaluation = "
          "%d%\n",
          eval);
}
