...
int sys_check_LRU_lists(uint32 *active_list_content,
  ...
  if (active_list_content != NULL) {
    ...
    LIST_FOREACH(ptr_WS_element, &(env->ActiveList)) {
      if (ROUNDDOWN(ptr_WS_element->virtual_address, PAGE_SIZE) !=
          ROUNDDOWN(active_list_content[idx_active_list], PAGE_SIZE)) {
  ...
  if (second_list_content != NULL) {
    ...
    LIST_FOREACH(ptr_WS_element, &(env->SecondList)) {
      if (ROUNDDOWN(ptr_WS_element->virtual_address, PAGE_SIZE) !=
          ROUNDDOWN(second_list_content[idx_second_list], PAGE_SIZE)) {
...
int sys_check_LRU_lists_free(uint32 *list_content, int list_size) {
  ...
  LIST_FOREACH(ptr_WS_element, &(env->ActiveList)) {
    for (int var = 0; var < list_size; var++) {
      if (ROUNDDOWN(ptr_WS_element->virtual_address, PAGE_SIZE) ==
          ROUNDDOWN(list_content[var], PAGE_SIZE)) {
...
int sys_check_WS_list(uint32 *WS_list_content, int actual_WS_list_size,
                      uint32 last_WS_element_content, bool chk_status) {
#if USE_KHEAP
  //	cprintf("CURRENT WS CONTENT BEFORE CHECKING:\n");
  ...
  if (last_WS_element_content != 0) {
    if (ROUNDDOWN(env->page_last_WS_element->virtual_address, PAGE_SIZE) !=
        ROUNDDOWN(last_WS_element_content, PAGE_SIZE)) {
      return WS_list_validation = 0;
    }
  }
  ...
  if (chk_status == 1) {
    ...
    //PAGE_SIZE) != ROUNDDOWN(WS_list_content[idx_WS_list], PAGE_SIZE))
    //			{
    ...
    if (last_WS_element_content) {
      for (int i = 0; i < actual_WS_list_size; ++i) {
        if (ROUNDDOWN(WS_list_content[i], PAGE_SIZE) ==
            ROUNDDOWN(last_WS_element_content, PAGE_SIZE)) {
          idx_WS_list = i;
          break;
        }
    ...
    for (int i = 0; i < actual_WS_list_size; ++i) {
      if (ROUNDDOWN(ptr_WS_element->virtual_address, PAGE_SIZE) !=
          ROUNDDOWN(WS_list_content[idx_WS_list], PAGE_SIZE)) {
        WS_list_validation = 0;
        break;
      }
    ...
    for (int idx_expected_list = 0; idx_expected_list < actual_WS_list_size;
         ++idx_expected_list) {
      bool found = 0;
      LIST_FOREACH(ptr_WS_element, &(env->page_WS_list)) {
        if (ROUNDDOWN(ptr_WS_element->virtual_address, PAGE_SIZE) ==
...
#else
  panic("sys_check_WS_list: this function is intended to be used when "
        "USE_KHEAP = 1");
  return 0;
#endif
}
