...
void fos_scheduler(void) {
  ...
  do {
    // Enable interrupts on this processor for a while to allow BLOCKED process
    // to resume The most recent process to run may have had interrupts turned
    // off; enable them to avoid a deadlock if all processes are waiting.
    sti();
    ...
    acquire_spinlock(
        &(ProcessQueues
              .qlock)); // lock: to protect ready & blocked Qs in multi-CPU
    // cprintf("ACQUIRED\n");
    do {
      // Get next env according to the current scheduler
      next_env = sched_next[scheduler_method]();

      // temporarily set the curenv by the next env JUST for checking the
      // scheduler Then: reset it again
      ...
      if (next_env != NULL) {
        // cprintf("\nScheduler select program '%s' [%d]... clock counter =
        ...
        // This is to avoid clock interrupt inside the scheduler after sti() of
        // the outer loop
        kclock_stop();
        // cprintf("\n[IEN = %d] clock is stopped! returned to scheduler after
        // context_switch. curenv = %d\n", (read_eflags() & FL_IF) == 0? 0:1,
        // curenv == NULL? 0 : curenv->env_id);

        // Process is done running for now. It should have changed its p->status
        // before coming back.
    ...
    release_spinlock(&ProcessQueues.qlock); // release lock: to protect ready &
                                            // blocked Qs in multi-CPU
    // cprintf("\n[FOS_SCHEDULER] release: lock status after = %d\n",
    // qlock.locked);

  } while (is_any_blocked > 0);

  /*2015*/ // No more envs... curenv doesn't exist any more! return back to
           // command prompt
  {
...
void sched_init_RR(uint8 quantum) {
  ...
  uint16 cnt0 = kclock_read_cnt0_latch(); // read after write to ensure it's set
...
//======================================
void sched_init_PRIRR(uint8 numOfPriorities, uint8 quantum,
                      uint32 starvThresh) {
  ...
  quantums = kmalloc(num_of_ready_queues * sizeof(uint8));
  quantums[0] = quantum;
  kclock_set_quantum(quantum); // millisecond??
  // locks
...
struct Env *fos_scheduler_RR() {
  ...
  // is runnable.
  /*To protect process Qs (or info of current process) in
   * multi-CPU************************/
  if (!holding_spinlock(&ProcessQueues.qlock))
    panic("fos_scheduler_RR: q.lock is not held by this CPU while it's "
          "expected to be.");
  /****************************************************************************************/
  struct Env *next_env = NULL;
...
struct Env *fos_scheduler_MLFQ() {
   ...
   * multi-CPU************************/
  if (!holding_spinlock(&ProcessQueues.qlock))
    panic("fos_scheduler_MLFQ: q.lock is not held by this CPU while it's "
          "expected to be.");
  /****************************************************************************************/
...
struct Env *fos_scheduler_BSD() {
   ...
   * multi-CPU************************/
  if (!holding_spinlock(&ProcessQueues.qlock))
    panic("fos_scheduler_BSD: q.lock is not held by this CPU while it's "
          "expected to be.");
  /****************************************************************************************/
...
struct Env *fos_scheduler_PRIRR() {
   ...
   * multi-CPU************************/
  if (!holding_spinlock(&ProcessQueues.qlock))
    panic("fos_scheduler_PRIRR: q.lock is not held by this CPU while it's "
          "expected to be.");
  /****************************************************************************************/
  // TODO: [PROJECT'24.MS3 - #08] [3] PRIORITY RR Scheduler -
  // fos_scheduler_PRIRR Your code is here Comment the following line panic("Not
  // implemented yet");
  struct Env *cur_proc = get_cpu_proc();
  // cprintf("GOWA FUNCTION EL SCHED\n");

  if (cur_proc != NULL) // cpu is not empty
  {

    // cur_proc->enqueued_time=ticks;
    // cprintf("ABL INSERT ENV %d\n",cur_proc->env_id);
    sched_insert_ready(cur_proc);
    // cprintf("BA3D INSERT ENV %d\n",cur_proc->env_id);
  }
  ...
  //			{	cprintf("QUEUE %d\n",i);
  //				LIST_FOREACH(iter,(&ProcessQueues.env_ready_queues[i]))
  //				{
  //					cprintf("id %d\n",iter->env_id);
  //				}
  //				i++;
  ...
  for (int i = 0; i < num_of_ready_queues; i++) {
    ...
    if (size != 0) {
      // struct Env* next_proc=LIST_FIRST(&ProcessQueues.env_ready_queues[i]);
      struct Env *next_proc = dequeue(&(ProcessQueues.env_ready_queues[i]));
      // remove_from_queue(&ProcessQueues.env_ready_queues[i],next_proc);
      kclock_set_quantum(quantums[0]);
      return next_proc;
    }
  }
  return NULL; //???cur proc
}
...
void clock_interrupt_handler(struct Trapframe *tf) {
  ...
  if (isSchedMethodPRIRR()) {
    // TODO: [PROJECT'24.MS3 - #09] [3] PRIORITY RR Scheduler -
    // clock_interrupt_handler Your code is here Comment the following line
    // panic("Not implemented yet");
    // int ticks=timer_ticks();
    // cprintf("clk function \n");
    struct Env *iter;
    int i = 0;
    //		while(i<num_of_ready_queues)
    //		{	cprintf("QUEUE %d\n",i);
    ...
    for (int i = 0; i < num_of_ready_queues; i++) {
      // cprintf("gowa for f clock \n");
      // cprintf("THRESHOLD IS %d\n",star_thres);
      if (i == 0)
        continue;
      struct Env *env = LIST_LAST(&ProcessQueues.env_ready_queues[i]);
      if (env == NULL) {
        continue;
      }

      //			cprintf("ticks of proc %d is
      //%d\n",env->env_id,ticks-env->enqueued_time);
      while (ticks - env->enqueued_time >= star_thres) //???=
      {
        // cprintf("gowa while f clock \n");
        dequeue(&(ProcessQueues.env_ready_queues[i]));
        // remove_from_queue(&ProcessQueues.env_ready_queues[i],env);
        // cprintf("ba3d dequeue f clock \n");
        // env->enqueued_time=ticks;
        // env->priority--;
        // enqueue(&(ProcessQueues.env_ready_queues[i-1]),env);
        // acquire_spinlock(&(ProcessQueues.qlock));
        // LIST_INSERT_TAIL(&(ProcessQueues.env_ready_queues[i-1]),env);
        // release_spinlock(&(ProcessQueues.qlock));
        // cprintf("ba3d enqueue f clock \n");
        //				if(i==0)
        //				{
        //					i=1;
        //				}
        env_set_priority(env->env_id, i - 1);
        env = LIST_LAST(&(ProcessQueues.env_ready_queues[i]));
        if (env == NULL) {
          break;
        }
      }
  ...
  if (curr_env_ptr != NULL) {
    struct WorkingSetElement *wse;
    {
      ...
      for (i = 0; i < (curr_env_ptr->page_WS_max_size); i++) {
        ...
        uint32 perm =
            pt_get_page_permissions(curr_env_ptr->env_page_directory, page_va);
        uint32 oldTimeStamp = wse->time_stamp;

        if (perm & PERM_USED) {
          wse->time_stamp = (oldTimeStamp >> 2) | 0x80000000;
          pt_set_page_permissions(curr_env_ptr->env_page_directory, page_va, 0,
                                  PERM_USED);
        } else {
      ...
      for (t = 0; t < __TWS_MAX_SIZE; t++) {
        if (curr_env_ptr->__ptr_tws[t].empty != 1) {
          ...
          if (pd_is_table_used(curr_env_ptr->env_page_directory, table_va)) {
            curr_env_ptr->__ptr_tws[t].time_stamp =
