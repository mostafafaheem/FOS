...
void sched_remove_ready(struct Env *env) {
  ...
  assert(env != NULL && env->env_status == ENV_READY);
  {
    for (int i = 0; i < num_of_ready_queues; i++) {
      struct Env *ptr_env =
          find_env_in_queue(&(ProcessQueues.env_ready_queues[i]), env->env_id);
      if (ptr_env != NULL) {
...
void sched_new_env(struct Env *e) {
  ...
  release_spinlock(
      &(ProcessQueues.qlock)); // CS on Qs
                               // cprintf("\n[SCHED_NEW_ENV] release: lock
                               // status after = %d\n", qlock.locked);
}
  ...
  LIST_FOREACH(ptr_env, &ProcessQueues.env_new_queue) {
    if (ptr_env->env_id == envId) {
      ...
      if (mycpu()->scheduler_status == SCH_STOPPED) {
        release_spinlock(&(
            ProcessQueues.qlock)); // CS on Qs
                                   // cprintf("\n[SCHED_RUN_ENV] release#1: lock
                                   // status after = %d\n", qlock.locked);
        fos_scheduler();
      } else {
        // can be invoked from a running environment via sys_run_env(), so just
        // release the lock and resume
      }
      break;
    }
  }
...
void sched_exit_env(uint32 envId) {
  ...
  if (found) {
    ...
    // If it's the curenv, then reinvoke the scheduler as there's no meaning to
    // return back to an exited env. Status already set to EXIT in the
    // sched_insert_exit() It's the fos_scheduler task to release the lock on
    // the Qs after context_switch to it from this process
    if (cur_env->env_id == envId) {
...
void sched_kill_env(uint32 envId) {
  ...
  if (!found) {
    ...
    LIST_FOREACH(ptr_env, &ProcessQueues.env_exit_queue) {
      if (ptr_env->env_id == envId) {
        cprintf("killing[%d] %s from the EXIT queue...", ptr_env->env_id,
                ptr_env->prog_name);
  ...
  if (found) {
    ...
    if (cur_env->env_id == envId) {
      ...
      cprintf("killing a RUNNABLE environment [%d] %s...", ptr_env->env_id,
              ptr_env->prog_name);
      env_free(ptr_env);
      cprintf("DONE\n");
      found = 1;
      // If it's the curenv, then reset it and reinvoke the scheduler as there's
      // no meaning to return back to a killed env. Status already set to EXIT
      // in the env_free() It's the fos_scheduler task to release the lock on
      // the Qs after context_switch to it from this process
...
void sched_run_all() {
  ...
  /*2023: Changed from LIST_FOREACH into DEQUEUE (based on suggestion from T52 &
   * T73 2023.Term1) to move the processes in FIFO order instead of LIFO in case
   * of LIST_FOREACH
   * */
...
void sched_kill_all() {
  ...
  if (cur_env) {
    ...
    cprintf("DONE\n");
    // If it's the curenv, then reset it and reinvoke the scheduler as there's
    // no meaning to return back to a killed env. Status already set to EXIT in
    // the env_free() It's the fos_scheduler task to release the lock on the Qs
    // after context_switch to it from this process reinvoke the scheduler since
    // there're no env to return back to it
    /*2024: replaced by sched() to apply context_switch*/
    sched();
  }
  release_spinlock(&(ProcessQueues.qlock)); // CS on Qs
...
int64 timer_ticks() { return ticks; }
...
void env_set_priority(int envID, int priority) {
  ...
  // Comment the following line
  // panic("Not implemented yet");
  if (proc->env_status == ENV_READY) // if ready
  {
    acquire_spinlock(&(ProcessQueues.qlock));
    sched_remove_ready(proc);
    proc->priority = priority;
    proc->enqueued_time = ticks;
    sched_insert_ready(proc);
    release_spinlock(&(ProcessQueues.qlock));
  } else {
    // cprintf("ELSE IN SET pri\n\n");
    proc->priority = priority;
  }
}

void sched_set_starv_thresh(uint32 starvThresh) {
  // TODO: [PROJECT'24.MS3 - #06] [3] PRIORITY RR Scheduler -
  // sched_set_starv_thresh Your code is here Comment the following line
  // panic("Not implemented yet");
