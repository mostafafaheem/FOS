...
uint8 *ptr_zero_page; // Virtual address of zero page used by program loader to
                      // initialize extra segment zero memory (bss section) it
                      // to zero
uint8 *ptr_temp_page; // Virtual address of a page used by program loader to
                      // initialize segment last page fraction
...
// struct FrameInfo* disk_frames_info;	// Virtual address of physical
// frames_info array
...
struct {
  ...
  struct FrameInfo_List
      modified_frame_list; // Modified frame list for buffering
