...
static inline void setUHeapPlacementStrategyFIRSTFIT() {
  _UHeapPlacementStrategy = UHP_PLACE_FIRSTFIT;
}
static inline void setUHeapPlacementStrategyBESTFIT() {
  _UHeapPlacementStrategy = UHP_PLACE_BESTFIT;
}
static inline void setUHeapPlacementStrategyNEXTFIT() {
  _UHeapPlacementStrategy = UHP_PLACE_NEXTFIT;
}
static inline void setUHeapPlacementStrategyWORSTFIT() {
  _UHeapPlacementStrategy = UHP_PLACE_WORSTFIT;
}

static inline uint8 isUHeapPlacementStrategyFIRSTFIT() {
  if (_UHeapPlacementStrategy == UHP_PLACE_FIRSTFIT)
    return 1;
  return 0;
}
static inline uint8 isUHeapPlacementStrategyBESTFIT() {
  if (_UHeapPlacementStrategy == UHP_PLACE_BESTFIT)
    return 1;
  return 0;
}
static inline uint8 isUHeapPlacementStrategyNEXTFIT() {
  if (_UHeapPlacementStrategy == UHP_PLACE_NEXTFIT)
    return 1;
  return 0;
}
static inline uint8 isUHeapPlacementStrategyWORSTFIT() {
...
uint32 memory_scarce_threshold_percentage; // Memory remains plentiful till the
                                           // % of free frames gets below this
                                           // threshold percentage
#define DEFAULT_MEM_SCARCE_PERCENTAGE                                          \
  25 // Default threshold % of free memory to indicate scarce MEM
//***********************************
...
/*2016*/ void *create_page_table(uint32 *ptr_page_directory,
...
// 2016
#define CHECK_IF_KERNEL_ADDRESS(virtual_address)                               \
  ((uint32)virtual_address >= (uint32)USER_TOP &&                              \
   (uint32)virtual_address <= (uint32)0xFFFFFFFF)
