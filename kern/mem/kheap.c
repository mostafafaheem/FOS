...
// Initialize the dynamic allocator of kernel heap with the given start address,
// size & limit All pages in the given range should be allocated Remember: call
// the initialize_dynamic_allocator(..) to complete the initialization Return:
//	On success: 0
...
int initialize_kheap_dynamic_allocator(uint32 daStart,
                                       uint32 initSizeToAllocate,
                                       uint32 daLimit) {
  // TODO: [PROJECT'24.MS2 - #01] [1] KERNEL HEAP -
  ...
  if (brk > daLimit) {
    ...
    for (int i = 0; i < num_pages; i++) {
      ...
      if (ret != E_NO_MEM) {
        map_frame(ptr_page_directory, ptr_frame_info, (uint32)va,
...
void *kmalloc(unsigned int size) {
    ...
    if (size <= DYN_ALLOC_MAX_BLOCK_SIZE && size > 0) {
      ...
      while (increment < num_of_frames) {
        ...
        if (PDX(pageCounter) != PDX(prevPageCounter)) {
          get_page_table(ptr_page_directory, (uint32)pageCounter,
                         &fisrtFitPageTable);
        }

        ptr_frame_info =
            get_frame_info(ptr_page_directory, pageCounter, &fisrtFitPageTable);
      ...
      for (int i = 0; i < num_of_frames; i++) {
        ...
        if (ret != E_NO_MEM) {
          ...
          map_frame(ptr_page_directory, framePointer, mapCounter,
                    PERM_WRITEABLE);
  ...
  // use "isKHeapPlacementStrategyFIRSTFIT() ..." functions to check the current
  // strategy
  return NULL;
}
...
void kfree(void *virtual_address) {
  ...
  if ((uint32)virtual_address < KERNEL_HEAP_START ||
    ...
    struct FrameInfo *ptr_frame_info =
        get_frame_info(ptr_page_directory, (uint32)virtual_address, &PageTable);
    int frames = ptr_frame_info->NumofFrames;
    for (int i = 0; i < frames; i++) {

      ptr_frame_info =
          get_frame_info(ptr_page_directory,
                         (uint32)(ptr_to_va + (i * PAGE_SIZE)), &PageTable);
...
unsigned int kheap_physical_address(unsigned int virtual_address) {
  ...
  if (get_page_table(ptr_page_directory, virtual_address, &pagetable) ==
      TABLE_NOT_EXIST) {
    return 0;
    ...
    if ((!(pagetableentry & PERM_PRESENT)) || (frame_inf == NULL) ||
        (frame_inf->references == 0))
      return 0;
...
//	Attempts to resize the allocated space at "virtual_address" to
//"new_size" bytes, 	possibly moving it in the heap. 	If successful, returns the
//new virtual_address, if moved to another loc: the old virtual_address must no
//longer be accessed. 	On failure, returns a null pointer, and the old
//virtual_address remains valid.
...
void *krealloc(void *virtual_address, uint32 new_size) {
  ...
  ptr_page_info = get_frame_info(ptr_page_directory, (uint32)virtual_address,
                                 &ptr_page_table);
  ...
  if (virtual_address == NULL && new_size == 0)
  ...
  else if ((uint32)virtual_address >= KERNEL_HEAP_START &&
           (uint32)virtual_address < rlimit_heap &&
           (new_size + 2 * sizeof(uint32)) == get_block_size(virtual_address))
    return virtual_address;

  else if ((uint32)virtual_address >= KERNEL_HEAP_START &&
           (uint32)virtual_address < rlimit_heap &&
           new_size <= DYN_ALLOC_MAX_BLOCK_SIZE && new_size > 0)
    return realloc_block_FF((void *)virtual_address, new_size);

  else if ((uint32)virtual_address >= KERNEL_HEAP_START &&
           (uint32)virtual_address < rlimit_heap &&
           new_size > DYN_ALLOC_MAX_BLOCK_SIZE) {
    ...
    return ret;
           ...
           ROUNDUP(new_size, PAGE_SIZE) ==
               (ptr_page_info->NumofFrames * PAGE_SIZE))
    return virtual_address;
    ...
    for (int i = 0; i < newNumOfFrames; i++) {
      if (PDX((uint32)virtual_address + i * PAGE_SIZE) !=
          PDX((uint32)virtual_address + (i - 1) * PAGE_SIZE))
        get_page_table(ptr_page_directory,
                       (uint32)virtual_address + i * PAGE_SIZE,
                       &ptr_page_table);

      ptr_page_info = get_frame_info(ptr_page_directory,
                                     (uint32)virtual_address + i * PAGE_SIZE,
                                     &ptr_page_table);
    ...
    for (int i = 0; i < extraFrames; i++) {
      if (PDX((uint32)virtual_address + newNumOfFrames + i * PAGE_SIZE) !=
          PDX((uint32)virtual_address + newNumOfFrames + (i - 1) * PAGE_SIZE))
        get_page_table(ptr_page_directory,
                       (uint32)virtual_address + i * PAGE_SIZE,
                       &ptr_page_table);

      ptr_page_info = get_frame_info(ptr_page_directory,
                                     (uint32)virtual_address + newNumOfFrames +
                                         i * PAGE_SIZE,
                                     &ptr_page_table);
      ptr_page_info->NumofFrames = 0;
      // acquire_spinlock(&MemFrameLists.mfllock);
      unmap_frame(ptr_page_directory,
                  (uint32)virtual_address + newNumOfFrames + (i * PAGE_SIZE));
      // release_spinlock(&MemFrameLists.mfllock);
    }
    return virtual_address;
  } else if ((uint32)virtual_address >= (rlimit_heap + PAGE_SIZE) &&
             ...
             ROUNDUP(new_size, PAGE_SIZE) >
                 (ptr_page_info->NumofFrames * PAGE_SIZE)) {
    get_page_table(ptr_page_directory, (uint32)virtual_address,
                   &ptr_page_table);
    ptr_page_info = get_frame_info(ptr_page_directory, (uint32)virtual_address,
                                   &ptr_page_table);
    ...
    while (iterations < extraFrames) {
      ...
      ptr_page_info =
          get_frame_info(ptr_page_directory, pageCounter, &ptr_page_table);
