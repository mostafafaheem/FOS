...
inline struct WorkingSetElement *
env_page_ws_list_create_element(struct Env *e, uint32 virtual_address) {
  ...
  // cprintf("CREATE WS ELEMENT\n");
  struct WorkingSetElement *ws = kmalloc(sizeof(struct WorkingSetElement));
  virtual_address = ROUNDDOWN(virtual_address, PAGE_SIZE);
  if (ws == NULL) {
    panic("kmalloc cannot allocate\n");
  }
  ws->virtual_address = virtual_address;
  // cprintf("CREATE WS ELEMENT DAKHAL IF\n");
  // cprintf("Allocating new WS element at virtual address: 0x%x\n",
  // virtual_address);
  return ws;
  // Your Code is Here...
  // return 0;
...
inline void env_page_ws_invalidate(struct Env *e, uint32 virtual_address) {
  if (isPageReplacmentAlgorithmLRU(PG_REP_LRU_LISTS_APPROX)) {
      ...
      if (ROUNDDOWN(ptr_WS_element->virtual_address, PAGE_SIZE) ==
        ...
        if (ptr_tmp_WS_element != NULL) {
          ...
          pt_set_page_permissions(e->env_page_directory,
                                  ptr_tmp_WS_element->virtual_address,
                                  PERM_PRESENT, 0);
        }
        found = 1;
        break;
    ...
    LIST_FOREACH(wse, &(e->page_WS_list)) {
      if (ROUNDDOWN(wse->virtual_address, PAGE_SIZE) ==
          ROUNDDOWN(virtual_address, PAGE_SIZE)) {
...
void env_page_ws_print(struct Env *e) {
  if (isPageReplacmentAlgorithmLRU(PG_REP_LRU_LISTS_APPROX)) {
    ...
    LIST_FOREACH(wse, &(e->page_WS_list)) {
      ...
      uint32 perm =
          pt_get_page_permissions(e->env_page_directory, virtual_address);
...
inline void env_page_ws_invalidate(struct Env *e, uint32 virtual_address) {
  int i = 0;
  for (; i < e->page_WS_max_size; i++) {
    if (ROUNDDOWN(e->ptr_pageWorkingSet[i].virtual_address, PAGE_SIZE) ==
        ROUNDDOWN(virtual_address, PAGE_SIZE)) {
...
inline void env_page_ws_set_entry(struct Env *e, uint32 entry_index,
  ...
  e->ptr_pageWorkingSet[entry_index].virtual_address =
      ROUNDDOWN(virtual_address, PAGE_SIZE);
...
inline uint32 env_page_ws_get_virtual_address(struct Env *e,
                                              uint32 entry_index) {
  assert(entry_index >= 0 && entry_index < (e->page_WS_max_size));
  ...
  if (isPageReplacmentAlgorithmLRU(PG_REP_LRU_LISTS_APPROX)) {
    ...
    for (i = 0; i < (e->page_WS_max_size); i++) {
      ...
      cprintf(", used= %d, modified= %d, buffered= %d, time stamp= %x, "
              ...
              e->ptr_pageWorkingSet[i].sweeps_counter);
...
inline void env_table_ws_invalidate(struct Env *e, uint32 virtual_address) {
  int i = 0;
  for (; i < __TWS_MAX_SIZE; i++) {
    if (ROUNDDOWN(e->__ptr_tws[i].virtual_address, PAGE_SIZE * 1024) ==
...
inline void env_table_ws_set_entry(struct Env *e, uint32 entry_index,
  ...
  assert(virtual_address >= 0 && virtual_address < USER_TOP);
  e->__ptr_tws[entry_index].virtual_address =
      ROUNDDOWN(virtual_address, PAGE_SIZE * 1024);
...
void half_WS_Size(struct Env *e, int isImmidiate) { panic("not handled yet"); }
