...
inline struct FrameInfo **create_frames_storage(int numOfFrames) {
  ...
  struct FrameInfo **frames_storage =
      (struct FrameInfo **)kmalloc(allocatedSize);
...
struct Share *create_share(int32 ownerID, char *shareName, uint32 size,
  ...
  //						//cprintf("4
  //HARRYYYYYYYYYYYYYYYYYYYYYYYY\n"); 						int ret = allocate_frame(&framePointer);
  //						if(ret != E_NO_MEM){
  //						framePointer->NumofFrames=num_of_frames;
  //						map_frame(ptr_page_directory,framePointer,mapCounter,PERM_WRITEABLE);
  //						uint32 physical_address =
  //to_physical_address(framePointer); 						uint32
  //frame_no=physical_address/PAGE_SIZE; 						frame_arr[frame_no]=mapCounter;
  //						//cprintf("frame NO:
  //%d\n",frame_no);
  //						//cprintf("VA page:
  //%x\n",frame_arr[frame_no]); 						mapCounter=mapCounter+ PAGE_SIZE; 						}else{
  //							//cprintf("NOOOO MEM FOR
  //FRAMESSSS\n");
  //						}
  //			 }
...
int createSharedObject(int32 ownerID, char *shareName, uint32 size,
  ...
  // TODO: [PROJECT'24.MS2 - #19] [4] SHARED MEMORY [KERNEL SIDE] -
  // createSharedObject() COMMENT THE FOLLOWING LINE BEFORE START CODING
...
int getSharedObject(int32 ownerID, char *shareName, void *virtual_address) {
  // TODO: [PROJECT'24.MS2 - #21] [4] SHARED MEMORY [KERNEL SIDE] -
  // getSharedObject() COMMENT THE FOLLOWING LINE BEFORE START CODING
  ...
  for (int i = 0; i < numofframes; i++) {
    ...
    if (iter->isWritable == 1) {
      // cprintf("if f for\n");
      map_frame(myenv->env_page_directory, (iter->framesStorage[i]), va,
                PERM_WRITEABLE | PERM_USER | PERM_PRESENT);
      // cprintf("b3d map\n");
    } else {
      // cprintf("else f  for\n");
      map_frame(myenv->env_page_directory, (iter->framesStorage[i]), va,
                PERM_USER | PERM_PRESENT);
    }
    // cprintf("a5r al for\n");
    va += PAGE_SIZE;
  }
...
void free_share(struct Share *ptrShare) {
  // TODO: [PROJECT'24.MS2 - BONUS#4] [4] SHARED MEMORY [KERNEL SIDE] -
  // free_share() COMMENT THE FOLLOWING LINE BEFORE START CODING
...
int freeSharedObject(int32 sharedObjectID, void *startVA) {
  // TODO: [PROJECT'24.MS2 - BONUS#4] [4] SHARED MEMORY [KERNEL SIDE] -
  // freeSharedObject() COMMENT THE FOLLOWING LINE BEFORE START CODING
  ...
  int size = getSizeOfSharedObject(shared_to_be_removed->ownerID,
                                   shared_to_be_removed->name);
