...
void acquire_spinlock(struct spinlock *lk) {
  ...
  pushcli(); // disable interrupts to avoid deadlock (in case if interrupted
             // from a higher priority (or event handler) just after holding the
             // lock => the handler will stuck in busy-waiting and prevent the
             // other from resuming)

  // cprintf("\nAttempt to acquire SPIN lock [%s] by [%d]\n", lk->name, myproc()
  // != NULL? myproc()->env_id : 0);
  ...
  for (i = 0; i < 10; i++) {
    ...
    if (ebp == 0 || (ebp < (uint32 *)USER_LIMIT) ||
        (ebp >= (uint32 *)(c->stack + KERNEL_STACK_SIZE) &&
         ebp < (uint32 *)(c->stack + KERNEL_STACK_SIZE + PAGE_SIZE)) ||
