...
///===================================================================================
struct Env *
get_cpu_proc(void); // get the the current running process on the CPU
...
void env_init(void);
/*Create new environment, initialize it, load the EXE into its memory and adjust
 * its address space*/
struct Env *env_create(char *user_program_name, unsigned int page_WS_size,
                       unsigned int LRU_second_list_size,
                       unsigned int percent_WS_pages_to_remove);
/*Free (delete) the environment by freeing its allocated memory and other
 * resources (if any)*/
...
void sched(void); // enter the fos_scheduler while keeping the interrupt status
                  // of the process and restoring it after return
void switchuvm(struct Env *proc); // switch to the user virtual memory (TSS &
                                  // Process Directory)
...
void env_exit(
    void); // add the running env to the EXIT queue, then reinvoke the scheduler
