...
struct Command commands[] = {
    ...
    {"modbufflength", "set the length of the modified buffer",
     command_set_modified_buffer_length, 1},
    {"setStarvThr", "set the the starvation threshold of priority scheduler",
     command_set_starve_thresh, 1},

    //******************************//
    /* COMMANDS WITH TWO ARGUMENTS */
    //******************************//
    {"wm", "writes one byte to specific physical location", command_writemem_k,
     2},
    {"schedBSD", "switch the scheduler to BSD with given # queues & quantum",
     command_sch_BSD, 2},
    {"setPri", "set the priority of the given environment (by its ID)",
     command_set_priority, 2},
    {"nclock",
     "set replacement algorithm to Nth chance CLOCK (type=1: NORMAL Ver. "
     "type=2: MODIFIED Ver.",
     command_set_page_rep_nthCLOCK, 2},

    //********************************//
    /* COMMANDS WITH THREE ARGUMENTS */
    //********************************//
    {"rub", "reads block of bytes from specific location in given environment",
     command_readuserblock, 3},
    // TODO: [PROJECT'24.MS3 - #07] [3] PRIORITY RR Scheduler - initialize
    // command
    {"schedPRIRR", "Switch & initialize the scheduler to to PRIORITY RR",
     command_sched_init_PRIRR, 3},
    ...
    {"wum",
     "writes one byte to specific location in kernel or a given environment",
     command_writeusermem, -1},
    {"rum",
     "reads one byte from specific location in kernel or a given environment",
     command_readusermem, -1},
...
int command_kernel_info(int number_of_arguments, char **arguments) {
  ...
  cprintf("  Start Address of the kernel 			%08x (virt)  "
          "%08x (phys)\n",
          start_of_kernel, start_of_kernel - KERNEL_BASE);
  cprintf("  End address of kernel code  			%08x (virt)  "
          "%08x (phys)\n",
          end_of_kernel_code_section, end_of_kernel_code_section - KERNEL_BASE);
  cprintf("  Start addr. of uninitialized data section 	%08x (virt)  %08x "
          "(phys)\n",
          start_of_uninitialized_data_section,
          start_of_uninitialized_data_section - KERNEL_BASE);
  cprintf("  End address of the kernel   			%08x (virt)  "
          "%08x (phys)\n",
...
int command_writemem_k(int number_of_arguments, char **arguments) {
  unsigned char *address =
      (unsigned char *)strtol(arguments[1], NULL, 16) + KERNEL_BASE;
  int c, i = 0;
...
int command_allocuserpage(int number_of_arguments, char **arguments) {
  ...
  if (number_of_arguments == 3) {
    ...
    map_frame(env->env_page_directory, ptr_FrameInfo, va,
              PERM_WRITEABLE | PERM_USER);
  } else if (number_of_arguments == 4) {
    // Map this frame to the given user virtual address with the given
    // permission
    uint32 rw;
    if (arguments[3][0] == 'r' || arguments[3][0] == 'R')
      ...
      cprintf(
          "aup command: wrong permission (r/w)... will continue as writable\n");
      rw = PERM_WRITEABLE;
    }
...
int command_meminfo(int number_of_arguments, char **arguments) {
  ...
  cprintf("Total available frames = %d\nFree Buffered = %d\nFree Not Buffered "
          "= %d\nModified = %d\n",
          counters.freeBuffered + counters.freeNotBuffered + counters.modified,
          counters.freeBuffered, counters.freeNotBuffered, counters.modified);
...
struct Env *CreateEnv(int number_of_arguments, char **arguments) {
...
#if USE_KHEAP
  {
    ...
    case 5:
      if (!isPageReplacmentAlgorithmLRU(PG_REP_LRU_LISTS_APPROX)) {
        cprintf("ERROR: Current Replacement is NOT LRU LISTS, invalid number "
                "of args\nUsage: <command> <prog_name> <page_WS_size> "
                "[<LRU_second_list_size>] [<BSD_Sched_Nice>]\naborting...\n");
        return NULL;
      }
    ...
    case 3:
      if (isPageReplacmentAlgorithmLRU(PG_REP_LRU_LISTS_APPROX)) {
        cprintf("ERROR: Current Replacement is LRU LISTS, Please specify a "
                "working set size in the 3rd arg and LRU second list size in "
                "the 4th arg, aborting.\n");
        return NULL;
      }
      pageWSSize = strtol(arguments[2], NULL, 10);
      break;
    default:
      cprintf("ERROR: invalid number of args\nUsage: <command> <prog_name> "
              "<page_WS_size> [<LRU_second_list_size>] "
              "[<DYN_LOC_SCOPE_percent_WS_to_remove>]\naborting...\n");
      return NULL;
    ...
    if (pageWSSize > __PWS_MAX_SIZE) {
      cprintf("ERROR: size of WS must be less than or equal to %d... aborting",
              __PWS_MAX_SIZE);
      return NULL;
    }
#endif
    if (isPageReplacmentAlgorithmLRU(PG_REP_LRU_LISTS_APPROX)) {
      if (LRUSecondListSize > pageWSSize - 1) {
        cprintf("ERROR: size of LRU second list can't equal/exceed the size of "
                "the page WS... aborting\n");
        return NULL;
      }
    }
    assert(percent_WS_pages_to_remove >= 0 &&
           percent_WS_pages_to_remove <= 100);

    // assert(PRIRRSchedPriority >= 0 && PRIRRSchedPriority <
    // num_of_ready_queues);
  ...
  env = env_create(arguments[1], pageWSSize, LRUSecondListSize,
...
int command_run_program(int number_of_arguments, char **arguments) {
  ...
  //[3] Run the created environment by adding it to the "ready" queue then
  //invoke the scheduler to execute it
...
int command_set_page_rep_LRU(int number_of_arguments, char **arguments) {
  if (number_of_arguments < 2) {
    cprintf("ERROR: please specify the LRU Approx Type (1: TimeStamp Approx, "
            "2: Lists Approx), aborting...\n");
    return 0;
  }
  ...
  if (LRU_TYPE == PG_REP_LRU_TIME_APPROX) {
    ...
    cprintf(
        "Page replacement algorithm is now LRU with TimeStamp approximation\n");
  } else if (LRU_TYPE == PG_REP_LRU_LISTS_APPROX) {
    setPageReplacmentAlgorithmLRU(LRU_TYPE);
    cprintf("Page replacement algorithm is now LRU with LISTS approximation\n");
  } else {
    cprintf("ERROR: Invalid LRU Approx Type (1: TimeStamp Approx, 2: Lists "
            "Approx), aborting...\n");
    return 0;
  }
  return 0;
...
int command_print_sch_method(int number_of_arguments, char **arguments) {
  if (isSchedMethodMLFQ()) {
    ...
    cprintf("Scheduler is now set to PRIORITY RR with %d priorities & quantum "
            "= %d\n",
            num_of_ready_queues, quantums[0]);
  } else
...
int command_set_kheap_plac_CONTALLOC(int number_of_arguments,
                                     char **arguments) {
...
int command_set_modified_buffer_length(int number_of_arguments,
                                       char **arguments) {
  if (!isBufferingEnabled()) {
    cprintf("Buffering is not enabled. Please enable buffering to use the "
            "modified buffer.\n");
  } else if (!isModifiedBufferEnabled()) {
    cprintf("Modified Buffering is not enabled. Please enable modified "
            "buffering.\n");
  }
