    ...
    if (c < 0) {
      ...
      if (last_c != 9) {
        ...
        if (number_of_arguments > 1) {
          if ((strcmp(arguments[0], "run") != 0) &&
              ...
              (strcmp(arguments[0], "tst") !=
               0)) // to autocomplete only in case that the command take
                   // arguments and defined arguments (run & load & tst) only
            continue;
          ...
          strcpy(temp_buf, arguments[number_of_arguments -
                                     1]); // temp_buf contains the last argument
        }
          ...
          if (is_run_cmd) {
            for (int var = 0; var < NUM_USER_PROGS; ++var) {
              ...
              if (x == 0) {
                ...
                for (int var3 = 0; var3 < strlen(ptr_UserPrograms[var].name);
                     ++var3) {
        ...
        if (prefix_list_size > 0) {
          ...
          RoundAutoCompleteCommandWithTheSamePrefix(strlen(PrefixList[prev]),
                                                    PrefixList[prefix_list_idx],
                                                    buf, &i, &lastIndex);
        }
      }
    }
      ...
      if (last_command_idx == HISTORY_MAX) {
        for (int idx = 0; idx < HISTORY_MAX; idx++) {
          memcpy(command_history[idx], command_history[idx + 1], BUFLEN);
        }
...
void run_command_prompt() {
  ...
  while (1 == 1) {
    ...
    // ********** This DosKey supported readline function is a combined
    // implementation from **********
    // ********** 		Mohamed Raafat & Mohamed Yousry, 3rd year
    // students, FCIS, 2017		**********
    // ********** 				Combined, edited and modified by
    // TA\Ghada Hamed				**********
 ...
 * The only way to get into the prompt is via this function to ensure correct
 * re-initializations The following variables are used to clear the entire
 * content of the KERNEL STACK before getting into the prompt They're placed
 * globally (instead of locally) to avoid clearing them while they're in use
 * [el7 :)]
 */
int m;
char *p;
void get_into_prompt() {
  while (1) {
    ...
    //		//Make sure it's in the correct stack (i.e. KERN STACK below
    //KERN_BASE) 		assert(cur_esp < SCHD_KERN_STACK_TOP && cur_esp >=
    //SCHD_KERN_STACK_TOP - KERNEL_STACK_SIZE);

    // Reset ESP to the beginning of the SCHED KERNEL STACK of this CPU before
    // getting into the cmd prmpt
    ...
    // Clear the stack content to avoid any garbage data on it when getting back
    // into prompt
    ...
    {
      // memset((char*)schd_kern_stack_bottom, 0, SCHD_KERN_STACK_TOP -
      // schd_kern_stack_bottom);
    ...
    // Reset EBP to ZERO so that when calling the run_command_prompt() it pushes
    // ZERO into the stack
    write_ebp(0);
...
int execute_command(char *command_string) {
  ...
  if (ret == CMD_INVALID) {
    ...
    if (numOfFoundCmds != 1) {
      panic("command is found but the list is either empty or contains more "
            "than one command!");
    }
    ...
    LIST_FOREACH(cmd, &foundCommands) { cprintf("[%d] %s\n", i++, cmd->name); }
    cprintf("Please select the required command [1] to [%d] and press enter? "
    ...
    LIST_FOREACH(cmd, &foundCommands) { LIST_REMOVE(&foundCommands, cmd); }
  } else {
...
int process_command(int number_of_arguments, char **arguments) {
  ...
  for (int current_command = 0; current_command < NUM_OF_COMMANDS;
       current_command++) {
    ...
    int j = 0;
    int size_of_command = strlen(commands[current_command].name);
    // cprintf("commmand:%s\n",commands[current_command].name);
    // cprintf("firstloop\n");
    for (int char_of_arg = 0; char_of_arg < size; char_of_arg++) {
      // cprintf("secondloop\n");
      while (j <= size_of_command) {
        // cprintf("thirdloop\n");
        if (commands[current_command].name[j] == arguments[0][char_of_arg]) {
          cprintf("ifinloop\n");
          word[length_of_word] = arguments[0][char_of_arg];
          length_of_word++;
          j++;
          break;
        } else {
    ...
    // cprintf("word:%s\n",word);
    if (strcmp(word, arguments[0]) == 0) {
      if (LIST_SIZE(&foundCommands) == 0) {
        LIST_INSERT_HEAD(&foundCommands, &commands[current_command]);
      } else {
        LIST_INSERT_TAIL(&foundCommands, &commands[current_command]);
      }
    }
  }
  if (LIST_SIZE(&foundCommands) != 0) {
