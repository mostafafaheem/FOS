...
// 2018 Percentage of the pages to be removed from the WS [either for scarce RAM
// or Full WS]
#define DEFAULT_PERCENT_OF_PAGE_WS_TO_REMOVE                                   \
  10 // 10% of the loaded pages is required to be removed
...
struct Env {
  ...
  /*MAIN INFO...*/
  //================
  struct Trapframe *env_tf; // Saved registers during the trap (at the top of
                            // the user kernel stack)
  struct Context
      *context; // Saved registers for context switching (env <--> scheduler)
                // (below the trap frame at the user kernel stack)
  ...
  char prog_name[PROGNAMELEN]; // Program name (to print it via USER.cprintf in
                               // multitasking)
  void *channel; // Address of the channel that it's blocked (sleep) on it
  uint32 enqueued_time;
  //================
  /*ADDRESS SPACE*/
  //================
  ...
  struct WorkingSetElement
      *page_last_WS_element; // ptr to last inserted WS element
#else
  struct WorkingSetElement ptr_pageWorkingSet[__PWS_MAX_SIZE];
  // uint32 page_last_WS_index;
  struct WS_List PageWorkingSetList; // LRU Approx: List of available WS
                                     // elements
#endif
  uint32 page_last_WS_index;
