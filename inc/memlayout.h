 ...
 *    4 GB --------->  +------------------------------+
 * 					   |      Invalid Memory (*)	  |
 *PAGE_SIZE KERNEL_HEAP_MAX ->  +------------------------------+ |     Kernel
 *Heap (KHEAP)      | RW/-- :              .               : :              . :
 ...
 * VPT,       -------> +------------------------------+ 0xefc00000
 *-----------------+ SCHD_KERN_STACK_TOP | Sched Kernel Stack CPU0      | RW/--
 *KERNEL_STACK_SIZE   		 . | Sched Kernel Stack CPU1      | RW/--
 *KERNEL_STACK_SIZE   		 .
 *      			   + ...
 *+									 . |
 *| 			  				   PTSIZE | Invalid
 ...
 *| 	             				 . USER_LIMIT  ------>
 *+------------------------------+ 0xef800000     ------------------+ |  Cur.
 *Page Table (User R-)   | R-/R-  	PTSIZE UVPT      ---->
 *+------------------------------+ 0xef400000 |   		  FREE Space
 *| 		PTSIZE
 *					   +------------------------------+
 *0xef000000 |           RO ENVS            | R-/R-  PTSIZE USER_TOP,UENVS -->
 *+------------------------------+ 0xeec00000 UXSTACKTOP -/       |     User
 *Exception Stack     | RW/RW  PAGE_SIZE
 *                     +------------------------------+ 0xeebff000
 ...
 * USER_HEAP_MAX,-->   ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 0xA0000000 [Original
 *Value 0xC0000000] USER_PAGES_WS_START .                              . . . .
 *User Heap         . USER_HEAP_START-->  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 *0x80000000 .                              . .                              .
 *                     .                              .
 *                     |     Program Code & Data 	  |
 *    UTEXT -------->  +------------------------------+ 0x00800000
...
// which maps all the page_table_entries containing the page mappings for the
// entire virtual address space into that 4 Meg region starting at VPT.
#define VPT (KERNEL_BASE - PTSIZE)
#define KERN_STACK_TOP VPT // scheduler kernel stacks (one per CPU)
#define KERNEL_STACK_SIZE                                                      \
  (8 * PAGE_SIZE) // size of a kernel stack (either for cpu scheduler stack or
                  // user kernel stack)
#define USER_LIMIT (KERN_STACK_TOP - PTSIZE)
#define NCPUS 1

/*
 * User read-only mappings! Anything below here til USER_TOP are readonly to
...
#define USER_PAGES_WS_MAX                                                      \
  (USER_PAGES_WS_START +                                                       \
   sizeof(struct WorkingSetElement) * USER_TOP / PAGE_SIZE)

#define USTACKBOTTOM (ROUNDUP(USER_PAGES_WS_MAX, PAGE_SIZE))

// 2022
#define USER_DYN_BLKS_ARRAY                                                    \
  0 //(ROUNDDOWN(USER_HEAP_START - (sizeof(struct MemBlock) *
    //NUM_OF_UHEAP_PAGES), PAGE_SIZE) - PAGE_SIZE)
 ...
 * page_table_entries can be accessed through a "virtual page table" at virtual
 * address VPT (to which vpt is set in entry.S).  The page_table_entry for page
 * number N is stored in vpt[N].  (It's worth drawing a diagram of this!)
 *
