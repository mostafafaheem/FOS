...
#define PTSIZE                                                                 \
  (PAGE_SIZE * NPTENTRIES) // bytes mapped by a page directory entry
...
// For each CPU which executes processes possibly wanting to do system calls via
// interrupts, one TSS is required. The only interesting fields are SS0 and
// ESP0. Whenever a system call occurs, the CPU gets the SS0 and ESP0-value in
// its TSS and assigns the stack-pointer to it.
struct Taskstate {
...
//			The difference is that for Interrupt Gates, interrupts
//are automatically disabled upon entry and reenabled upon IRET, 			whereas this
//does not occur for Trap Gates.
