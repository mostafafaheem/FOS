...
static __inline void cpuid(uint32 info, uint32 *eaxp, uint32 *ebxp,
                           uint32 *ecxp, uint32 *edxp);
...
// Ahmed 2010:
static __inline void lcr2(uint32 val) {
...
static __inline void write_esp(uint32 esp) {
  __asm __volatile("movl %0,%%esp" : : "r"(esp));
}

static __inline void write_ebp(uint32 ebp) {
...
static __inline uint64 read_tsc(void) {
  uint64 tsc;
