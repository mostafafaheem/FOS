...
void fixedPt2Str(fixed_point_t f, int num_dec_digits, char *output) {
  ...
  char intPart[20];
  ltostr(integer, intPart);
  char fractPart[20];
  ltostr(fraction, fractPart);
...
void sys_utilities(char *utilityName, int value) {
  ...
  } else if (strncmp(utilityName, "__CheckExitOrder@",
                     strlen("__CheckExitOrder@")) == 0) {
...
void scarce_memory() {
  uint32 total_size_tobe_allocated =
      ((100 - memory_scarce_threshold_percentage) * number_of_frames) / 100;
  ...
  acquire_spinlock(&MemFrameLists.mfllock);
  {
    ...
    uint32 size_tobe_allocated =
        total_size_tobe_allocated - size_of_already_allocated;
...
uint32
calc_no_pages_tobe_removed_from_ready_exit_queues(uint32 WS_or_MEMORY_flag) {
...
#endif
          int num_of_pages_to_be_removed =
              cur_env->percentage_of_WS_pages_to_be_removed *
              num_of_pages_in_WS / 100;
          if ((cur_env->percentage_of_WS_pages_to_be_removed *
               num_of_pages_in_WS) %
                  100 >
              0)
...
#endif
      int num_of_pages_to_be_removed =
          cur_env->percentage_of_WS_pages_to_be_removed * num_of_pages_in_WS /
          100;
      if ((cur_env->percentage_of_WS_pages_to_be_removed * num_of_pages_in_WS) %
              100 >
          0)
        num_of_pages_to_be_removed++;
  ...
  return no_of_pages_tobe_removed_from_curenv +
         no_of_pages_tobe_removed_from_ready +
         no_of_pages_tobe_removed_from_exit;
}
...
void chk1() {
  ...
  //"NULL" : __pe->prog_name, __pl); 		schenv();
  //	}
}
void chk2(struct Env *__se) {
    ...
    if (__firsttime) {
      ...
      if (timer_ticks() % 100 == 0) {
        ...
        if (__nnexit == __pnexit) {
          // cprintf("++++++++++++++++++# processes = %d, prev la = %d.%d, next
  ...
  //current, lower, upper); 			assert_endall(current > lower && current <= upper) ;
  //
  //			for (int i = 0; i < num_of_ready_queues; ++i)
  //			{
  //				assert_endall(find_env_in_queue(&(env_ready_queues[i]),
  //__ne->env_id) == NULL);
  //			}
  //		}
  //		if (__pe != NULL && __pe != __ne)
  //		{
  //			uint8 __tl = __pl == num_of_ready_queues-1 ? __pl : __pl
  //+ 1 ; 			assert_endall(find_env_in_queue(&(env_ready_queues[__tl]),
  //__pe->env_id) != NULL); 			for (int i = 0; i < num_of_ready_queues; ++i)
  //			{
  //				if (i == __tl) continue;
  //				assert_endall(find_env_in_queue(&(env_ready_queues[i]),
  //__pe->env_id) == NULL) ;
...
void check_boot_pgdir() {
  ...
  // space 	n = ROUNDUP(number_of_frames*sizeof(struct Frame_Info), PAGE_SIZE);
  //	for (i = 0; i < n; i += PAGE_SIZE)
  //	{
  //		//cprintf("i = %x, arg 1  = %x, arg 2 = %x \n",i,
  //check_va2pa(ptr_page_directory, READ_ONLY_FRAMES_INFO + i),
  //STATIC_KERNEL_PHYSICAL_ADDRESS(frames_info) + i);
  //		assert(check_va2pa(ptr_page_directory, READ_ONLY_FRAMES_INFO +
  //i) == STATIC_KERNEL_PHYSICAL_ADDRESS(frames_info) + i);
  //	}
  ...
  for (i = 0; i < NCPUS * KERNEL_STACK_SIZE; i += PAGE_SIZE) {
    ...
    if (i % KERNEL_STACK_SIZE == 0)
      continue;
    assert(check_va2pa(ptr_page_directory,
                       KERN_STACK_TOP - NCPUS * KERNEL_STACK_SIZE + i) ==
           STATIC_KERNEL_PHYSICAL_ADDRESS(ptr_stack_bottom) + i);
  }
  ...
  for (i = 0; i < NPDENTRIES; i++) {
    ...
    case PDX(UENVS):
      // 2016: READ_ONLY_FRAMES_INFO not valid any more since it can't fit in 4
      // MB space case PDX(READ_ONLY_FRAMES_INFO):
...
// performs this functionality for us!  We define our own version to help check
...
void page_check()
{
        ...
        free_frame(pp0);
        assert(map_frame(ptr_page_directory, pp1, 0x0, 0) == 0);
        assert(EXTRACT_ADDRESS(ptr_page_directory[0]) ==
to_physical_address(pp0)); assert(check_va2pa(ptr_page_directory, 0x0) ==
to_physical_address(pp1)); assert(pp1->references == 1); assert(pp0->references
== 1);

        // should be able to map pp2 at PAGE_SIZE because pp0 is already
allocated for page table assert(map_frame(ptr_page_directory, pp2, (void*)
PAGE_SIZE, 0) == 0); assert(check_va2pa(ptr_page_directory, PAGE_SIZE) ==
to_physical_address(pp2)); assert(pp2->references == 1);
        ...
        assert(check_va2pa(ptr_page_directory, PAGE_SIZE) ==
to_physical_address(pp2)); assert(pp2->references == 1);
        ...
        // should not be able to map at PTSIZE because need free frame for page
table assert(map_frame(ptr_page_directory, pp0, (void*) PTSIZE, 0) < 0);
...
to_physical_address(pp1));
        ...
        assert(check_va2pa(ptr_page_directory, PAGE_SIZE) ==
to_physical_address(pp1)); assert(pp1->references == 1); assert(pp2->references
== 0);
        ...
        assert(EXTRACT_ADDRESS(ptr_page_directory[0]) ==
to_physical_address(pp0)); if(USE_KHEAP)
        {
