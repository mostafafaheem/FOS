...
struct Test tests[] = {
    {"3functions",
     ...
     tst_three_creation_functions},
    {"kfreeall", "Kernel Heap: test kfreeall (freed frames, mem access...etc)",
     tst_kfreeall},
    ...
    {"kfreelast", "Kernel Heap: test freeing last allocated var",
     tst_kfreelast},
    {"priority1", "Tests the priority of the program (Normal and Higher)",
     tst_priority1},
    {"priority2", "Tests the priority of the program (Normal and Lower)",
     tst_priority2},
    {"mlfq_sc4", "Scenario#4: MLFQ", tst_sc_MLFQ},
    {"bsd_nice",
     "BSD Scheduler: check order of running multiple instances of same program "
    ...
    {"pg", "Test paging manipulation for a specific page",
     tst_paging_manipulation},
...
int tst_kheap(int number_of_arguments, char **arguments) {
  ...
  if (strcmp(arguments[2], "kmalloc") == 0 && number_of_arguments != 4) {
    cprintf("Invalid number of arguments! USAGE: tst kheap <Strategy> kmalloc "
            "<1 or 2 or 3>\n");
    return 0;
  }
  if (strcmp(arguments[2], "kmalloc") != 0 && number_of_arguments != 3) {
    if (strcmp(arguments[2], "krealloc") != 0 && number_of_arguments != 5) {
      cprintf("Invalid number of arguments! USAGE: tst kheap <Strategy> "
              "<testname>\n");
      return 0;
    }
  }
  ...
  } else if (strcmp(arguments[1], "BF") == 0 ||
             strcmp(arguments[1], "bf") == 0) {
    setKHeapPlacementStrategyBESTFIT();
    cprintf("Kernel Heap placement strategy is BEST FIT\n");
  } else if (strcmp(arguments[1], "NF") == 0 ||
             strcmp(arguments[1], "nf") == 0) {
  ...
  if (strcmp(arguments[2], "kmalloc") == 0) {
    ...
    if (isKHeapPlacementStrategyFIRSTFIT()) {
      if (testNum == 0) {
        cprintf("Error: [Kernel.FirstFit] must specify the test number (1 or "
                "2) as an argument\n");
        return 0;
      }
      ...
      if (testNum == 0) {
        cprintf("Error: [Kernel.BestFit] must specify the test number (1, 2, "
                "3) as an argument\n");
        return 0;
      }
      if (testNum == 1)
      ...
      if (testNum == 0) {
        cprintf("Error: [Kernel.NextFit] must specify the test number (1 or 2) "
                "as an argument\n");
        return 0;
      }
