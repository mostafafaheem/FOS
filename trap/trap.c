...
static void trap_dispatch(struct Trapframe *tf) {
  if (tf->tf_trapno == T_PGFLT) {
    // 2016: Bypass the faulted instruction [used for some tests in which we
    // need to resume the execution after an intended page fault]
    if (bypassInstrLength != 0) {
      tf->tf_eip = (uint32 *)((uint32)(tf->tf_eip) + bypassInstrLength);
      /*2024: commented. already will be returned to the trapret() in
       * trapentry.S which return to the user/kernel caller code*/
      // kclock_resume();
      // env_pop_tf(tf);
      return;
    }
    ...
    /* If the original status of the interrupt is ENABLED (before getting into
     * kernel), Then, re-enable the interrupts & resume the clock during the
     * system calls to allow switching between processes
     */
    ...
    /*If still enabled, Disable the interrupt & stop clock before getting into
     * user again
     */
...
void trap(struct Trapframe *tf) {
   ...
   * NOTE: interrupt is automatically disabled by the interrupt cycle (by
   * marking all traps as "Interrupt Gates"). Resume the clock and Re-enable the
   * interrupt whenever required (e.g. in system calls).
   */
  kclock_stop();
  ...
  int userTrap = 0;
  struct Env *cur_env =
      get_cpu_proc(); // the current running Environment (if any)

  if ((tf->tf_cs & 3) == 3) {
    assert(cur_env && cur_env->env_status ==
                          ENV_RUNNING); // environment should be exist & run
    // cprintf("curenv->env_tf @ %x, tf param @ %x\n", curenv->env_tf , tf);
    assert(cur_env->env_tf == tf); // tf should be placed in the kernel stack of
                                   // this process (@e->env_tf)
    userTrap = 1;
  }
  //	cprintf("\n[%s - %d] trap #%d - %s  @va = %x - eip = %x - IEN = %d\n",
  //userTrap == 1? "USER" : "KERNEL", userTrap == 1? curenv->env_id : 0,
  //tf->tf_trapno, trapname(tf->tf_trapno), tf, tf->tf_eip, (read_eflags() &
  //FL_IF) == 0? 0 : 1); 	if (tf->tf_trapno == T_SYSCALL)
  //	{
  ...
  /*2024: will be returned to the trapret() in trapentry.S which return to the
   * caller*/
  ...
  //kclock_read_cnt0_latch()); 	cprintf("[tf] tf @%x - tf.cs = %x - tf.eip = %x -
