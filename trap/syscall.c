...
// Lock the console so that no other processes can read from KB or output to the
// monitor
void sys_lock_cons(void) { cons_lock(); }
// Unlock the console so that other processes can read from KB or output to the
// monitor
void sys_unlock_cons(void) { cons_unlock(); }
...
static int __sys_allocate_page(void *va, int perm) {
...
#if USE_KHEAP
  {
    ...
    //		we are using an unsed VA in the invalid area of kernel at
    //0xef800000 (the current USER_LIMIT) 		to do temp initialization of a frame.
    ...
    // the frame we just got from allocate_frame, we will use it for the new
    // page
...
uint32 sys_calculate_required_frames(uint32 start_virtual_address,
                                     uint32 size) {
...
uint32 sys_calculate_free_frames() {
  ...
  //counters.freeBuffered + counters.freeNotBuffered, counters.freeBuffered
...
void sys_scarce_memory(void) { scarce_memory(); }
...
void sys_free_user_mem(uint32 virtual_address, uint32 size) {
  ...
  if ((void *)virtual_address == NULL) {
    env_exit();
  } else if (USER_HEAP_START > virtual_address ||
...
void sys_allocate_user_mem(uint32 virtual_address, uint32 size) {
  ...
  if ((void *)virtual_address == NULL) {
    // cprintf("NULL VIRTUAL ADDRESS! \n");
    env_exit();
  } else if (virtual_address < USER_HEAP_START ||
             virtual_address >= USER_HEAP_MAX) {
    // cprintf("out of the boundaries! \n");
    env_exit();
  } else {
    // cprintf("hanendah allocate_user_mem \n");
...
// 2015
uint32 sys_get_heap_strategy() { return _UHeapPlacementStrategy; }
...
void sys_enqueue(struct Env_Queue *queue, struct semaphore *sem,
  ...
  // cprintf("dakhal sys_enqueue\n");
  acquire_spinlock(&(ProcessQueues.qlock));
  sem->semdata->lock = 0;
  env->env_status = ENV_BLOCKED;
  enqueue(queue, env); // law mashtghaletshh neb3at env fe syscall
  sched();
  sem->semdata->lock = 1;
  release_spinlock(&(ProcessQueues.qlock));
  // cprintf("dakhal sys_enqeue w aamal release l el lk\n");

  return;
}
void sys_dequeue(struct Env_Queue *queue, struct semaphore *sem) {
  // cprintf("dakhal sysCall insert\n");
  acquire_spinlock(&(ProcessQueues.qlock));
  sem->semdata->lock = 0;
  struct Env *e = dequeue(queue);
  sched_insert_ready0(e);
  e->env_status = ENV_READY;
  sem->semdata->lock = 1;
  release_spinlock(&(ProcessQueues.qlock));
  // struct Env* e=dequeue(queue);
}
void sys_pushcli(void) {
  pushcli();
  return;
}
void sys_popcli(void) {
  popcli();
  return;
}
void sys_sched_insert_ready0(struct Env *env, struct semaphore *sem) {
  //	cprintf("dakhal sysCall insert\n");
  //	acquire_spinlock(&(ProcessQueues.qlock));
  //	//sem->semdata->lock=0;
...
int sys_createSharedObject(char *shareName, uint32 size, uint8 isWritable,
  ...
  return createSharedObject(get_cpu_proc()->env_id, shareName, size, isWritable,
                            virtual_address);
}
...
// 2017
static int32 sys_getenvid(void) { return cur_env->env_id; }
...
// 2017
static int32 sys_getparentenvid(void) { return cur_env->env_parent_id; }

// Destroy a given environment whatever its state & DON'T place it in EXIT
// if envid=0, destroy the currently running environment --> schedule the next
// (if any)
//
...
static void sys_exit_env() {
  ...
  // 2024: if returned here, then it's not the current environment. So, just
  // return env_run_cmd_prmpt(); context_switch(&(cur_env->context),
  // mycpu()->scheduler);
}
...
int sys_create_env(char *programName, unsigned int page_WS_size,
  ...
  struct Env *env = env_create(programName, page_WS_size, LRU_second_list_size,
...
void sys_run_env(int32 envId) { sched_run_env(envId); }
...
uint32 sys_rcr2() { return rcr2(); }
void sys_bypassPageFault(uint8 instrLength) { bypassInstrLength = instrLength; }
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4,
  ...
  case SYS_enqueue:
    ...
    break;
  case SYS_dequeue:
    sys_dequeue(((struct Env_Queue *)a1), (struct semaphore *)a2);
    return 0;
    break;
  case SYS_pushcli:
    sys_pushcli();
  ...
  case SYS_popcli:
    ...
    return 0;
    break;
  case SYS_sched_insert_ready0:
    sys_sched_insert_ready0((struct Env *)a1, (struct semaphore *)a2);
    return 0;
    break;
  case NSYSCALLS:
    return -E_INVAL;
