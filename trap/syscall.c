...
void sys_free_user_mem(uint32 virtual_address, uint32 size)
{
	  ...
	  else if(USER_HEAP_START > virtual_address || virtual_address>= USER_HEAP_MAX)
			        {
			            env_exit();
			        }else{
...
void sys_enqueue(struct Env_Queue* queue,struct semaphore* sem,struct Env* envv)
{
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	case SYS_init_queue:
		...
		break;
	case SYS_enqueue:
		sys_enqueue((struct Env_Queue*)a1,(struct semaphore*)a2,(struct Env*)a3);
		return 0;
		break;
	case SYS_dequeue:
