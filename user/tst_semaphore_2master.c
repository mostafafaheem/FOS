// Test the use of semaphores to allow multiprograms to enter the CS at same
// time Master program: take user input, create the semaphores, run slaves and
// wait them to finish
#include <inc/lib.h>
...
void _main(void) {
  ...
  struct semaphore shopCapacitySem =
      create_semaphore("shopCapacity", shopCapacity);
  ...
  for (; i < totalNumOfCusts; i++) {
    ...
    if (id == E_ENV_CREATION_ERROR)
      panic("NO AVAILABLE ENVs... Please reduce the number of customers and "
            "try again...");
    sys_run_env(id);
  }
  ...
  if (sem2val == 0 && sem1val == shopCapacity)
    cprintf("\nCongratulations!! Test of Semaphores [2] completed "
            "successfully!!\n\n\n");
  else
    cprintf("\nError: wrong semaphore value... please review your semaphore "
            "code again...\n");
