...
void _main(void) {
  ...
  // sys_createSharedObject("customers", sizeof(struct Customer)*numOfCustomers,
  // 1, (void**)&custs);
  ...
  *flight2BookedCounter = 0;
  ...
  int *flight1BookedArr =
      smalloc(_flightBooked1Arr, sizeof(int) * flight1NumOfTickets, 1);
  int *flight2BookedArr =
      smalloc(_flightBooked2Arr, sizeof(int) * flight2NumOfTickets, 1);

  int *cust_ready_queue =
      smalloc(_cust_ready_queue, sizeof(int) * numOfCustomers, 1);
  ...
  *queue_out = 0;
  ...
  struct semaphore *cust_finished = smalloc(
      "cust_finished_array", numOfCustomers * sizeof(struct semaphore), 1);
  ...
  for (c = 0; c < numOfCustomers; ++c) {
    ...
    if (envId == E_ENV_CREATION_ERROR)
      panic("NO AVAILABLE ENVs... Please reduce the num of customers and try "
            "again");
  ...
  // check out the final results and semaphores
  {
    ...
    int f3 = f2;
    for (; f3 < f2 + flight3Customers; ++f3) {
      if (find(flight1BookedArr, flight1NumOfTickets, f3) != 1 ||
          find(flight2BookedArr, flight2NumOfTickets, f3) != 1) {
        panic("Error, wrong booking for user %d\n", f3);
      }
    }
    ...
    for (s = 0; s < numOfCustomers; ++s) {
      ...
      //			ltostr(s, id);
      //			strcconcat(prefix, id,
      //cust_finishedSemaphoreName); 			assert(sys_getSemaphoreValue(envID,
      //cust_finishedSemaphoreName) ==  0);
      assert(semaphore_count(cust_finished[s]) == 0);
    }

    cprintf("Congratulations, All reservations are successfully done... have a "
            "nice flight :)\n");
  }
}
