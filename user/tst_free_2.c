...
void _main(void) {
  ...
  cprintf("3: Free some allocated blocks [Merge with next ONLY]\n\n");
  {
    ...
    if (check_block(startVAs[blockIndex], startVAs[blockIndex], expectedSize,
                    0) == 0) {
      is_correct = 0;
  ...
  cprintf("5: Allocate After Free [should be placed in coalesced blocks]\n\n");
  {
    ...
    cprintf("	5.1.2: b. after the prev alloc in 5.1.1\n\n");
    {
      ...
      expectedSize = ROUNDUP(actualSize + sizeOfMetaData, 2);
      // Check returned va
      expectedVA = (void *)(USER_HEAP_START + sizeOfMetaData + 4 * sizeof(int) +
                            sizeOfMetaData);
      if (check_block(va, expectedVA, expectedSize, 1) == 0) {
        is_correct = 0;
    ...
    cprintf("	5.1.3: c. between two blocks [INTERNAL FRAGMENTATION "
            "CASE]\n\n");
    {
      actualSize = 5 * sizeof(int); // 20B
      expectedSize =
          allocSizes[0] +
          allocSizes[1]; // 20B + 16B [Internal Fragmentation of 8 Bytes]
      va = malloc(actualSize);
      // Check returned va
    ...
    cprintf("	5.3.3: c. after the prev allocated block in 5.3.2\n\n");
    {
      ...
      // Check returned va
      expectedVA = (void *)startVAs[numOfAllocs * allocCntPerSize - 1] +
                   3 * kilo / 2 + sizeOfMetaData;
      if (check_block(va, expectedVA, expectedSize, 1) == 0) {
        is_correct = 0;
  ...
  cprintf("6: Check memory allocation [should not be changed due to free]\n\n");
  {
    ...
    if ((freeFrames - sys_calculate_free_frames()) != 0) {
      cprintf("test_free_2 #6: number of allocated pages in MEMORY is changed "
              "due to free() while it's not supposed to!\n");
      is_correct = 0;
    }
    if (is_correct) {
      eval += 10;
  ...
  uint32 expectedAllocNumOfPages =
      expectedAllocatedSize / PAGE_SIZE; /*# pages*/
  uint32 expectedAllocNumOfTables =
      ROUNDUP(expectedAllocatedSize, PTSIZE) / PTSIZE; /*# tables*/
  ...
  cprintf("7: Check WS Elements [should not be changed due to free]\n\n");
  {
    ...
    int i = 0;
    for (uint32 va = USER_HEAP_START;
         va < USER_HEAP_START + expectedAllocatedSize; va += PAGE_SIZE) {
      expectedVAs[i++] = va;
    }
    chk = sys_check_WS_list(expectedVAs, expectedAllocNumOfPages, 0, 2);
    if (chk != 1) {
      cprintf("test_free_2 #7: page is either not added to WS or removed from "
              "it\n");
      is_correct = 0;
    }
    if (is_correct) {
  ...
  cprintf("%~test free() with FIRST FIT completed [DYNAMIC ALLOCATOR]. "
          "Evaluation = %d%\n",
          eval);
