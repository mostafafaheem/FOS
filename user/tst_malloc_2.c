...
void _main(void) {
   ...
   * sizes*/
  cprintf("%~\n1: [BLOCK ALLOCATOR] allocate set of blocks with different "
          "sizes [all should fit] [30%]\n");
  {
    is_correct = 1;
  ...
  cprintf("%~\n2: Check stored data inside each allocated block [30%]\n");
  {
    ...
    for (int i = 0; i < idx; ++i) {
      if (*(startVAs[i]) != i || *(midVAs[i]) != i || *(endVAs[i]) != i) {
        is_correct = 0;
        cprintf("alloc_block_xx #2.%d: WRONG! content of the block is not "
                "correct. Expected %d\n",
                i, i);
        break;
      }
  ...
  uint32 expectedAllocNumOfPagesForWS =
      ROUNDUP(expectedAllocNumOfPages *
                  (sizeof(struct WorkingSetElement) + sizeOfMetaData),
              PAGE_SIZE) /
      PAGE_SIZE; /*# pages*/
  ...
  {
    is_correct = 1;
    uint32 expected = expectedAllocNumOfPages + expectedAllocNumOfTables +
                      expectedAllocNumOfPagesForWS;
    uint32 actual = (freeFrames - sys_calculate_free_frames());
    if (expected != actual) {
      cprintf("number of allocated pages in MEMORY not correct. Expected %d, "
              "Actual %d\n",
              expected, actual);
      is_correct = 0;
    }
