...
int QSort(int *Elements, int NumOfElements, int startIndex, int finalIndex,
          int kIndex);
...
void _main(void) {
  ...
  struct semaphore cons_mutex =
      get_semaphore(*consMutexOwnerID, "Console Mutex");
  ...
  shMean = smalloc("mean", sizeof(int64), 0);
  *shMean = mean;
  shVar = smalloc("var", sizeof(int64), 0);
  *shVar = var;
