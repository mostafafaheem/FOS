...
void _main(void) {
  cprintf("gwa al testttttttttttt\n");
  /*********************** NOTE ****************************
  ...
  uint32 *x, *y, *z;
  int diff, expected;
  cprintf("gwa b3d decalrationnnnnn\n");
  ...
  if (x != (uint32 *)pagealloc_start) {
    is_correct = 0;
    cprintf("Create(): Returned address is not correct. make sure that you "
            "align the allocation on 4KB boundary");
  }
  ...
  freeFrames = sys_calculate_free_frames();
  z = smalloc("z", 4, 1);
  if (z != (uint32 *)(pagealloc_start + 2 * PAGE_SIZE)) {
    is_correct = 0;
    cprintf("Create(): Returned address is not correct. make sure that you "
            "align the allocation on 4KB boundary");
  }
  ...
  *y = 20;
  ...
  if (*z != 30) {
    is_correct = 0;
    cprintf("Error!! Please check the creation (or the getting) of shared "
            "variables!!\n\n\n");
  }
