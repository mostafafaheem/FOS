...
void _main(void) {
  ...
  if (sem2val == 0 && sem1val == 1)
    cprintf("Congratulations!! Test of Semaphores [1] completed "
            "successfully!!\n\n\n");
  else
    panic("Error: wrong semaphore value... please review your semaphore code "
          "again! Expected = %d, %d, Actual = %d, %d",
          1, 0, sem1val, sem2val);
