 ...
 * & no later block can be allocated in a previous unused free block at the end
 * of page
 */

uint32 allocSizes[numOfAllocs] = {3 * sizeof(int) + sizeOfMetaData,
                                  2 * sizeOfMetaData,
                                  20 * sizeof(char) + sizeOfMetaData,
                                  kilo / 2,
                                  1 * kilo,
                                  3 * kilo / 2,
                                  2 * kilo};
...
void _main(void) {
  ...
  //====================================================================//
  /*INITIAL ALLOC Scenario 1: Try to allocate set of blocks with different
   * sizes*/
  cprintf("PREREQUISITE#1: Try to allocate set of blocks with different sizes "
          "[all should fit]\n\n");
  {
    is_correct = 1;
    ...
    for (int i = 0; i < numOfAllocs; ++i) {
      for (int j = 0; j < allocCntPerSize; ++j) {
        ...
        if (diff > 0 && diff < (DYN_ALLOC_MIN_BLOCK_SIZE + sizeOfMetaData)) {
          ...
          curVA =
              ROUNDUP(curVA, PAGE_SIZE) -
              sizeof(int) /*next alloc will start at END Block (after sbrk)*/;
  ...
  roundedTotalSize = ROUNDUP(curTotalSize, PAGE_SIZE);
  uint32 remainSize =
      (roundedTotalSize - curTotalSize) - sizeof(int) /*END block*/;
  ...
  cprintf("1: Free set of blocks with different sizes (first block of each "
          "size)\n\n");
  {
    is_correct = 1;
    for (int i = 0; i < numOfAllocs; ++i) {
...
#define numOfFFTests 3
   ...
   * free blocks*/
  cprintf("2: Try to allocate set of blocks with different sizes smaller than "
          "existing free blocks\n\n");

  uint32 testSizes[numOfFFTests] = {
      kilo / 4, // expected to be allocated in 4th free block
      8 * sizeof(char) +
          sizeOfMetaData, // expected to be allocated in 1st free block
...

  {
    ...
    for (int i = 0; i < numOfFFTests; ++i) {
      ...
      // Check returned va
      if (check_block(tstStartVAs[i], (void *)expectedVAs[i], expectedSizes[i],
                      1) == 0) {
        is_correct = 0;
    ...
    if (get_block_size(tstStartVAs[1]) != allocSizes[0]) {
      is_correct = 0;
      cprintf("test_ff_2 #2.3: WRONG FF ALLOC - make sure if the remaining "
              "free space doesn’t fit a dynamic allocator block, then this "
              "area should be added to the allocated area and counted as "
  ...
  cprintf("3: Try to allocate a block with equal to the first existing free "
          "block\n\n");
  {
    is_correct = 1;

    actualSize = kilo / 8 - sizeOfMetaData; // expected to be allocated in
                                            // remaining of 4th free block
    ...
    if (check_block(tstStartVAs[numOfFFTests], expectedVA, expectedSize, 1) ==
        0) {
      is_correct = 0;
  ...
  cprintf("4: Check stored data inside each allocated block\n\n");
  {
    ...
    for (int i = 0; i <= numOfFFTests; ++i) {
      ...
      if (*(tstStartVAs[i]) != (353 + i) || *(tstMidVAs[i]) != (353 + i) ||
          *(tstEndVAs[i]) != (353 + i)) {
        is_correct = 0;
        cprintf("malloc #4.%d: WRONG! content of the block is not correct. "
                "Expected=%d, val1=%d, val2=%d, val3=%d\n",
  ...
  cprintf("5: Test a Non-Granted Request\n\n");
  {
    ...
    uint32 numOfRem2KBAllocs =
        ((USER_HEAP_START + DYN_ALLOC_MAX_SIZE - (uint32)sbrk(0)) / PAGE_SIZE) *
        2;
    for (int i = 0; i < numOfRem2KBAllocs; ++i) {
      va = malloc(actualSize);
      if (va == NULL) {
        is_correct = 0;
        cprintf("malloc() #5.%d: WRONG FF ALLOC - alloc_block_FF return NULL "
                "address while it's expected to return correct one.\n");
        break;
      }
    }
    ...
    if (va != NULL) {
      is_correct = 0;
      cprintf("malloc() #6: WRONG FF ALLOC - alloc_block_FF return an address "
              "while it's expected to return NULL since it reaches the hard "
              "limit.\n");
    }
    if (is_correct) {
      eval += 20;
