...
void _main(void) {
  ...
  cprintf("STEP A: checking PLACEMENT fault handling... [40%] \n");
  {
    ...
    if (arr[PAGE_SIZE * 1024] != 2) {
      is_correct = 0;
      cprintf("PLACEMENT of stack page failed\n");
    }
    if (arr[PAGE_SIZE * 1025] != 2) {
      is_correct = 0;
      cprintf("PLACEMENT of stack page failed\n");
    }

    if (arr[PAGE_SIZE * 1024 * 2] != 3) {
      is_correct = 0;
      cprintf("PLACEMENT of stack page failed\n");
    }
    if (arr[PAGE_SIZE * 1024 * 2 + PAGE_SIZE] != 3) {
      is_correct = 0;
      cprintf("PLACEMENT of stack page failed\n");
    }
    ...
    if ((sys_pf_calculate_allocated_pages() - usedDiskPages) != 0) {
      is_correct = 0;
      cprintf("new stack pages should NOT be written to Page File until "
              "evicted as victim\n");
    }
    ...
    if ((freePages - sys_calculate_free_frames()) != expected) {
      is_correct = 0;
      cprintf("allocated memory size incorrect. Expected Difference = %d, "
              "Actual = %d\n",
  ...
  cprintf("STEP B: checking WS entries... [30%]\n");
  {
    ...
    if (found != 1) {
      is_correct = 0;
      cprintf("PAGE WS entry checking failed... trace it by printing page WS "
              "before & after fault\n");
    }
  }
  ...
  cprintf("STEP C: checking working sets WHEN BECOMES FULL... [30%]\n");
  {
    ...
    if (myEnv->page_last_WS_element != NULL) {
      is_correct = 0;
      cprintf("wrong PAGE WS pointer location... trace it by printing page WS "
              "before & after fault\n");
    }
    ...
    if (arr[PAGE_SIZE * 1024 * 3] != 4) {
      is_correct = 0;
      cprintf("PLACEMENT of stack page failed\n");
    }
    //		if( arr[PAGE_SIZE*1024*3 + PAGE_SIZE] !=  -1)  { is_correct = 0;
    //cprintf("PLACEMENT of stack page failed\n");}
    ...
    /*NO NEED FOR THIS IF REPL IS "LRU"*/
    // if(myEnv->page_last_WS_index != 0) { is_correct = 0; cprintf("wrong PAGE
    // WS pointer location... trace it by printing page WS before & after
    // fault\n");}
  }
