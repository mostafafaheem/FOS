...
void _main(void) {
  ...
  cprintf("STEP A: checking PLACEMENT fault handling ... \n");
  {
    ...
    if (arr[PAGE_SIZE * 1024] != 'A') {
      is_correct = 0;
      cprintf("PLACEMENT of stack page failed\n");
    }
    if (arr[PAGE_SIZE * 1025] != 'A') {
      is_correct = 0;
      cprintf("PLACEMENT of stack page failed\n");
    }

    if (arr[PAGE_SIZE * 1024 * 2] != 'A') {
      is_correct = 0;
      cprintf("PLACEMENT of stack page failed\n");
    }
    if (arr[PAGE_SIZE * 1024 * 2 + PAGE_SIZE] != 'A') {
      is_correct = 0;
      cprintf("PLACEMENT of stack page failed\n");
    }
  ...
  {
    int check =
        sys_check_LRU_lists(actual_active_list, actual_second_list, 17, 0);
    if (check == 0) {
      is_correct = 0;
  ...
  cprintf("Congratulations!! Test of PAGE PLACEMENT FIRST SCENARIO completed. "
          "Eval = %d\n\n\n",
          eval);
  return;
}
