...
uint32 expectedInitialVAs[11] = {
    ...
    0xeebfd000, /*0xedbfd000 will be created during the call of
                   sys_check_WS_list*/ // Stack
};
...
void _main(void) {
...
#if USE_KHEAP
  {
    ...
    if (found != 1)
      panic("INITIAL PAGE WS entry checking failed! Review size of the "
  ...
  // cprintf("Checking Allocation in Mem & Page File... \n");
  {
    if ((sys_pf_calculate_allocated_pages() - usedDiskPages) != 0)
      panic("Unexpected extra/less pages have been added to page file.. NOT "
            "Expected to add new pages to the page file");

    int freePagesAfter =
        (sys_calculate_free_frames() + sys_calculate_modified_frames());
    if ((freePages - freePagesAfter) != 0)
      panic("Extra memory are wrongly allocated... It's REplacement: expected "
            "that no extra frames are allocated. Expected = %d, Actual = %d",
            0, (freePages - freePagesAfter));
  }

  cprintf("Congratulations!! test PAGE replacement [ALLOCATION] is completed "
          "successfully\n");
  return;
}
