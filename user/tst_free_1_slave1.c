...
void _main(void) {
  ...
  // ALLOCATE ONE SPACE
  {
    // 2 MB
    {
      ...
      if (actualNumOfFrames < expectedNumOfFrames)
        panic("Wrong fault handler: pages are not loaded successfully into "
  ...
  // Test accessing a freed area but NOT ACCESSED Before (processes should be
  // killed by the validation of the fault handler)
  {
    byteArr[8 * kilo] = minByte;
    inctst();
    panic("tst_free_1_slave2 failed: The env must be killed and shouldn't "
          "return here.");
  }
