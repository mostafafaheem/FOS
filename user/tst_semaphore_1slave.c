...
// Slave program: enter critical section, print it's ID, exit and signal the
// master program
#include <inc/lib.h>
...
void _main(void) {
  ...
  wait_semaphore(cs1);
  {
    ...
    if (sem1val > 0)
      panic("Error: more than 1 process inside the CS... please review your "
            "semaphore code again...");
    env_sleep(1000);
  }
