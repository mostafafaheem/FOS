...
void _main(void) {
  ...
  for (int i = 0; i < numOfSlaves; ++i) {
    id = sys_create_env("tstChanAllSlave", (myEnv->page_WS_max_size),
                        (myEnv->SecondListSize),
                        (myEnv->percentage_of_WS_pages_to_be_removed));
    if (id == E_ENV_CREATION_ERROR) {
      cprintf("\n%~insufficient number of processes in the system! only %d "
              "slave processes are created\n",
              i);
      numOfSlaves = i;
      break;
    }
  ...
  while (numOfBlockedProcesses != numOfSlaves) {
    ...
    cnt++;
    if (cnt == numOfSlaves) {
      panic("%~test channels failed! unexpected number of blocked slaves. "
            "Expected = %d, Current = %d",
            numOfSlaves, numOfBlockedProcesses);
    }
    sys_utilities("__GetChanQueueSize__", (uint32)(&numOfBlockedProcesses));
  }
  ...
  cprintf("Congratulations!! Test of Channel (sleep & wakeup ALL) completed "
          "successfully!!\n\n\n");
